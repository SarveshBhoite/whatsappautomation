...
export default function LeadsDisplayPage() {
  ...
  const [displayCampaignName, setDisplayCampaignName] = useState<string>(`Leads-Display-${Date.now().toString().slice(-4)}`);
  ...
  useEffect(() => {
    ...
    try {
      ...
      console.warn("Could not parse googleAds_prefill_campaign for Leads Display:", err);
    }
  }, [customerId]);
            ...
            <button
              onClick={async () => {
                ...
                try {
                  ...
                  const res = await fetch(`${BACKEND}/api/ads/campaigns/leads/display`, {
                    method: "POST",
