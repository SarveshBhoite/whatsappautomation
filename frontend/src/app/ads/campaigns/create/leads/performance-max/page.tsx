...
export default function LeadsPerformanceMaxPage() {
  ...
  const [brandEntries, setBrandEntries] = useState<string[]>([""]);
              ...
              <div className="p-6 rounded-2xl border border-slate-800 bg-slate-900/90 space-y-6 shadow-xl">
                ...
                <div className="space-y-2 pt-3 border-t border-slate-800">
                  ...
                  <div className="flex flex-wrap items-center gap-3 pt-1">
                    ...
                    <button
                      type="button"
                      className="px-3.5 py-1.5 rounded-xl bg-primary/10 text-primary border border-primary/30 font-semibold text-xs hover:bg-primary/20 cursor-pointer"
                    >
                      Use brand lists to exclude brands
                    </button>
                  </div>
              ...
              <div className="p-6 rounded-2xl border border-slate-800 bg-slate-900/90 space-y-4 shadow-xl">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    ...
                    <span className="font-bold text-slate-100 text-sm">Leads-Performance Max-16</span>
                  </div>
                  <div>
          ...
          {wizardStep !== "SUMMARY" ? (
            ...
            <button
              onClick={async () => {
                ...
                alert(`Leads Performance Max campaign published successfully!`);
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-2xl max-w-lg w-full p-6 space-y-5 shadow-2xl text-xs">
            ...
            <div className="space-y-2">
              <div className="flex items-center justify-between">
                ...
                <span className="text-[11px] text-slate-500">Add brands to your list</span>
              </div>
              {brandEntries.map((brand, i) => (
                <div key={i} className="flex items-center gap-2">
                  ...
                  {brandEntries.length > 1 && (
                    <button
                      type="button"
                      onClick={() => setBrandEntries(prev => prev.filter((_, idx) => idx !== i))}
              ...
              <button
                type="button"
                onClick={() => setBrandEntries(prev => [...prev, ""])}
                className="inline-flex items-center gap-1 text-primary text-xs font-semibold hover:underline pt-1 cursor-pointer"
              >
                <Plus className="h-3.5 w-3.5" />
                Add another brand
            ...
            <div className="flex justify-end gap-3 pt-3 border-t border-slate-800">
              ...
              <button
                type="button"
                onClick={() => {
                  if (!newBrandListName.trim()) {
                    alert("Please enter a list name");
                    return;
                  }
                  const validBrands = brandEntries.filter(b => b.trim());
                  setCreatedBrandLists(prev => [...prev, { name: newBrandListName, brands: validBrands }]);
                  setNewBrandListName("");
                  setBrandEntries([""]);
