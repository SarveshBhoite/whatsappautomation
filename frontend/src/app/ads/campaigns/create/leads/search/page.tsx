...
export default function LeadsSearchPage() {
  ...
  const [campaignName, setCampaignName] = useState<string>("Leads-Search-1");
    ...
    fetch(`${BACKEND}/api/ads/campaigns?orgId=${encodeURIComponent(orgId)}&customerId=${encodeURIComponent(targetCid)}`)
      ...
      .then(data => {
        if (Array.isArray(data)) {
          ...
          // Auto-calculate next available index for Leads-Search-# if default is taken
          let nextIndex = 1;
          const existingLower = data.map((c: any) => (c.name || "").trim().toLowerCase());
          while (existingLower.includes(`Leads-search-${nextIndex}`.toLowerCase())) {
            nextIndex++;
          }
          const defaultName = `Leads-Search-${nextIndex}`;
              ...
              <div className="p-6 rounded-2xl border border-slate-200 bg-white space-y-4 shadow-sm">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <span className="font-semibold text-slate-900 block mt-0.5">Leads</span>
                  </div>
                  <div>
