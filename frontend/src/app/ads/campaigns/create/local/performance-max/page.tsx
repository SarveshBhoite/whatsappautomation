...
export default function LocalPerformanceMaxPage() {
  return (
    <Suspense fallback={<div className="p-8 text-xs text-slate-500">Loading campaign setup...</div>}>
      <LocalPerformanceMaxContent />
    </Suspense>
  );
}

function LocalPerformanceMaxContent() {
    ...
    try {
      ...
      await fetch(`${BACKEND}/api/ads/campaign/draft`, {
        ...
        body: JSON.stringify({
          ...
          geoTargets: typeof selectedLocation !== "undefined" && selectedLocation === "ALL"
            ? ["ALL"]
            : selectedLocation === "INDIA"
              ? ["INDIA"]
              : (selectedCustomLocations.length > 0 ? selectedCustomLocations : (customLocationInput ? [customLocationInput] : ["ALL"])),
          draftData: {
          ...
          if (Array.isArray(drafts)) {
            ...
            if (found) {
              ...
              if (Array.isArray(found.geoTargets) && found.geoTargets.length > 0) {
                if (found.geoTargets[0] === "ALL" || found.geoTargets[0] === "All countries and territories") {
                  setSelectedLocation("ALL");
                } else if (found.geoTargets[0] === "INDIA" || found.geoTargets[0] === "India") {
                  setSelectedLocation("INDIA");
                } else {
                  setSelectedLocation("CUSTOM");
                  if (typeof found.geoTargets[0] === "object") {
                    setSelectedCustomLocations(found.geoTargets as GeoTargetItem[]);
                  } else {
                    setSelectedCustomLocations(found.geoTargets.map((item: any) => typeof item === "string" ? ({
                      name: item,
                      canonicalName: item,
                      mode: "LOCATION"
                    }) : item));
              ...
              const dData = found.audienceSignal || found.draftData;
              if (dData && typeof dData === "object") {
                if (dData.selectedLocation) setSelectedLocation(dData.selectedLocation);
                if (Array.isArray(dData.selectedCustomLocations) && dData.selectedCustomLocations.length > 0) {
                  setSelectedLocation("CUSTOM");
                  setSelectedCustomLocations(dData.selectedCustomLocations);
                }
                if (dData.locationOptionsPresence) setLocationOptionsPresence(dData.locationOptionsPresence);
                if (dData.locationOptionsExclude) setLocationOptionsExclude(dData.locationOptionsExclude);
      ...
      try {
        if (typeof window !== "undefined") {
          ...
          if (prefillRaw) {
            ...
            if (prefill.storeLocations) {
              setStoreLocationsEnabled(true);
              if (typeof prefill.storeLocations === "object" && prefill.storeLocations.locationFilter) {
                setBusinessProfileLocationFilter(prefill.storeLocations.locationFilter);
              }
            }
            if (prefill.localCustomerOptimization !== undefined) {
              setLocalCustomerOptimization(Boolean(prefill.localCustomerOptimization));
            }
            if (Array.isArray(prefill.promotions) && prefill.promotions.length > 0) {
              setSavedPromotions(prefill.promotions.map((p: any) => ({
                occasion: p.occasion || "None",
                item: p.promotionTarget || p.item || "Special Offer",
                discount: String(p.percentOff || p.discount || "10"),
                url: p.finalUrl || p.url || prefill.finalUrl || prefill.website || ""
              })));
            }
          }
        }
      } catch (err) {
        console.warn("Could not parse googleAds_prefill_campaign for Local Store Visits PMax:", err);
      }
    }
  }, [customerId, draftId]);
