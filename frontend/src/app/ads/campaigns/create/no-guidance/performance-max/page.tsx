...
export default function NoGuidancePerformanceMaxPage() {
  return (
    <Suspense fallback={<div className="p-8 text-xs text-slate-500">Loading campaign setup...</div>}>
      <NoGuidancePerformanceMaxContent />
    </Suspense>
  );
}

function NoGuidancePerformanceMaxContent() {
  ...
  const [campaignName, setCampaignName] = useState<string>("NoGuidance-Performance Max-1");
    ...
    try {
      ...
      await fetch(`${BACKEND}/api/ads/campaign/draft`, {
        ...
        body: JSON.stringify({
          ...
          campaignName: campaignName || "NoGuidance-Performance Max-1",
  ...
  const [lfNoGuidancecoringQuestion, setLfNoGuidancecoringQuestion] = useState<string>("");
                        ...
                        <div className="flex items-center justify-between">
                          ...
                          <div className="flex items-center gap-2">
                            <button
                              type="button"
                              onClick={() => {
                                const aiGens = Array.from({ length: Math.max(headlines.length, 3) }, (_, i) => `AI Headline ${i + 1}: Boost NoGuidance ${i + 1}`);
                                setHeadlines(aiGens);
                              }}
              ...
              <div className="p-6 rounded-2xl border border-slate-200 bg-white space-y-4 shadow-sm">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div className="md:col-span-2">
                    ...
                    <span className="font-bold text-slate-900">NoGuidance (Downloads, Phone call NoGuidance)</span>
                  </div>
                </div>
              </div>
            ...
            <button
              ...
              onClick={async () => {
                ...
                try {
                  ...
                  const data = await res.json();
                  if (!res.ok) {
                    throw new Error(data.error || "Failed to publish NoGuidance Performance Max campaign.");
                  }

                  router.push(`/ads${customerId ? `?customerId=${customerId}` : ""}`);
                } catch (err: any) {
                  console.error("NoGuidance PMax campaign submission error:", err);
            ...
            <div className="space-y-5">
                  ...
                  <div className="space-y-2">
                    ...
                    >
                      ...
                      <optgroup label="Business">
                        ...
                        <option value="What is your annual NoGuidance volume?">What is your annual NoGuidance volume?</option>
              ...
              <div className="space-y-3 pt-3 border-t border-slate-200">
                ...
                <div className="p-3 bg-slate-50 border border-slate-200 rounded-xl text-[11px] text-slate-700 space-y-1">
                  <span className="font-semibold block">Download collected NoGuidance in ad extensions table</span>
                  <p className="text-slate-500">You can only download NoGuidance that've been collected within the last 30 days</p>
                </div>
              ...
              <div className="space-y-3 pt-3 border-t border-slate-200">
                <div>
                  ...
                  <p className="text-[11px] text-slate-500">Optimize for NoGuidance with higher intent or for more NoGuidance overall. <a href="https://support.google.com/google-ads/answer/10089406?hl=en_US#optimization" target="_blank" rel="noreferrer" className="text-primary hover:underline font-semibold">Learn more</a></p>
                </div>
                ...
                <div className="grid grid-cols-2 gap-3">
                  <label className={`p-3 rounded-xl border flex flex-col justify-between cursor-pointer transition-all ${lfFormType === "MORE_VOLUME" ? "border-primary bg-primary/10" : "border-slate-200 bg-slate-50 hover:bg-white"}`}>
                    ...
                    <p className="text-[10px] text-slate-500 mt-1">This may result in NoGuidance with lower intent.</p>
                  </label>
                  ...
                  <label className={`p-3 rounded-xl border flex flex-col justify-between cursor-pointer transition-all ${lfFormType === "MORE_QUALIFIED" ? "border-primary bg-primary/10" : "border-slate-200 bg-slate-50 hover:bg-white"}`}>
                    ...
                    <p className="text-[10px] text-slate-500 mt-1">This may result in fewer NoGuidance or a higher cost per lead.</p>
                  </label>
                </div>
              ...
              <div className="space-y-4 animate-in slide-in-from-right-3 duration-200">
                <div className="space-y-1">
                  ...
                  >
                    ...
                    {["All customers", "Purchasers", "High-value customers", "Disengaged customers", "Qualified NoGuidance", "Converted NoGuidance", "Paid subscribers", "Cart abandoners", "Loyalty program members", "Tier 1", "Tier 2", "Tier 3", "Tier 4", "Tier 5", "Tier 6", "Tier 7"].map(opt => (
