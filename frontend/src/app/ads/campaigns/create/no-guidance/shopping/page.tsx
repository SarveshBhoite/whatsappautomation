...
export default function NoGuidanceShoppingPage() {
  ...
  // Campaign Name
  const [campaignName, setCampaignName] = useState<string>("NoGuidance-Shopping-1");
  ...
  const [salesCountry, setSalesCountry] = useState<string>("IN");
    ...
    fetch(`${BACKEND}/api/ads/campaigns?orgId=${encodeURIComponent(orgId)}&customerId=${encodeURIComponent(targetCid)}`)
      ...
      .then(data => {
        if (Array.isArray(data)) {
          ...
          while (existingLower.includes(`noguidance-shopping-${nextIndex}`.toLowerCase())) {
            nextIndex++;
          }
          const defaultName = `NoGuidance-Shopping-${nextIndex}`;
        ...
        try {
          ...
          console.warn("Could not restore googleAds_prefill_campaign in No Guidance Shopping form:", err);
        }
      })
      .catch(() => {});
  }, [customerId]);
  ...
  const getReviewValidationErrors = (): ValidationIssue[] => {
    ...
    // 4. Shopping Settings (Merchant Center ID and Sales Country)
    ...
    const trimmedSalesCountry = (salesCountry || feedLabel || "").trim();
    if (!trimmedSalesCountry) {
      issues.push({
        id: "camp-sales-country-req",
        level: "Campaign",
        parameter: "Sales Country",
        message: "Sales Country is required.",
        step: "AD_GROUP",
        settingKey: "product_groups"
      });
    }
                  ...
                  <div className="space-y-4 pt-2 text-xs">
                    ...
                    <div className="p-4 border border-blue-500/30 bg-blue-500/5 rounded-xl space-y-3">
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 pt-1">
                        ...
                        <div className="space-y-1">
                          <label className="text-slate-700 font-semibold text-xs">Feed / Sales Country <span className="text-rose-500">*</span></label>
                          <select
                            value={salesCountry}
                            onChange={(e) => {
                              setSalesCountry(e.target.value);
              ...
              <div className="p-6 rounded-2xl border border-slate-200 bg-white space-y-4 shadow-sm">
                ...
                <div className="grid grid-cols-2 gap-y-4 text-xs">
                  ...
                  <div>
                    ...
                    <p className="text-slate-800 font-semibold mt-1">No Guidance</p>
                  </div>
                  <div>
                    <p className="text-slate-500 font-medium">Merchant Center and CSS</p>
                    <p className="text-slate-800 font-semibold mt-1">{merchantCenterId || "Not set"} ({salesCountry}) / CSS: Google Shopping</p>
                  </div>
                  <div>
                ...
                onClick={async () => {
                  ...
                  try {
                    ...
                    const sanitizedShoppingSetting = {
                      ...
                      salesCountry: salesCountry.trim(),
                      feedLabel: feedLabel.trim() || salesCountry.trim(),
                    ...
                    if (!payloadToLaunch.shoppingSetting || !payloadToLaunch.shoppingSetting.merchantId || !payloadToLaunch.shoppingSetting.salesCountry) {
                      setSubmitError("Shopping Setting is incomplete. Merchant Center ID and Sales Country are required.");
                      setIsPublishing(false);
                      return;
                    }

                    const res = await fetch(`${BACKEND}/api/ads/campaigns/no-guidance/shopping`, {
                      method: "POST",
