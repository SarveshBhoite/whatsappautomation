    ...
    fetch(`${BACKEND}/api/ads/campaigns?orgId=${encodeURIComponent(orgId)}&customerId=${encodeURIComponent(targetCid)}`)
      ...
      .then(data => {
        if (Array.isArray(data)) {
          ...
          // Auto-calculate next available index for NoGuidance-Search-# if default is taken
          let nextIndex = 1;
          const existingLower = data.map((c: any) => (c.name || "").trim().toLowerCase());
          while (existingLower.includes(`NoGuidance-search-${nextIndex}`.toLowerCase())) {
            nextIndex++;
          }
      ...
      <header className="h-14 bg-white border-b border-slate-200 px-4 sm:px-6 flex items-center justify-between shrink-0 sticky top-0 z-50">
        <div className="flex items-center gap-2 sm:gap-4">
          ...
          <div className="flex items-center gap-2 border-l border-slate-200 pl-3 sm:pl-4 text-xs font-semibold">
            <span className="text-slate-500 hidden sm:inline">NoGuidance</span>
        ...
        <main className="flex-1 p-6 md:p-10 overflow-y-auto space-y-6 max-w-4xl mx-auto">
          ...
          {wizardStep === "KEYWORDS_ADS" && (
            <div className="space-y-6 animate-in fade-in duration-200 text-xs">
              ...
              {/* Create ads to get more NoGuidance Section Header */}
              <div className="space-y-1 pt-4">
                <h2 className="text-base font-bold text-slate-900">Create ads to get more NoGuidance</h2>
              </div>
              ...
              <div className="p-6 rounded-2xl border border-slate-200 bg-white space-y-4 shadow-sm">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <span className="font-semibold text-slate-900 block mt-0.5">NoGuidance</span>
                  </div>
                  <div>
              ...
              <div className="p-6 rounded-2xl border border-slate-200 bg-white space-y-4 shadow-sm">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  ...
                  <div>
                    ...
                    <span className="font-medium text-slate-800">
                      {selectedLocation === "ALL" ? "All countries and territories" : selectedLocation === "INDIA" ? "India" : selectedCustomLocations.map(l => l.name).join(", ") || "Custom locations"}
                    </span>
                  </div>
                  <div>
