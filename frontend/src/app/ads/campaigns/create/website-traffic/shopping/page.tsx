...
export default function WebsiteTrafficShoppingPage() {
  ...
  // Campaign Name
  const [campaignName, setCampaignName] = useState<string>("WebsiteTraffic-Shopping-1");
  ...
  const [WebsiteTrafficCountry, setWebsiteTrafficCountry] = useState<string>("IN");
    ...
    fetch(`${BACKEND}/api/ads/campaigns?orgId=${encodeURIComponent(orgId)}&customerId=${encodeURIComponent(targetCid)}`)
      ...
      .then(data => {
        if (Array.isArray(data)) {
          ...
          while (existingLower.includes(`WebsiteTraffic-shopping-${nextIndex}`.toLowerCase())) {
            nextIndex++;
          }
          const defaultName = `WebsiteTraffic-Shopping-${nextIndex}`;
        ...
        try {
          ...
          if (prefillRaw) {
            ...
            if (prefill && (prefill.campaignType === "SHOPPING" || !prefill.campaignType)) {
              ...
              if (prefill.salesCountry || prefill.WebsiteTrafficCountry) {
                setWebsiteTrafficCountry(prefill.salesCountry || prefill.WebsiteTrafficCountry);
              }
          ...
          console.warn("Could not restore googleAds_prefill_campaign in Website Traffic Shopping form:", err);
        }
      })
      .catch(() => {});
  }, [customerId]);
  ...
  const getReviewValidationErrors = (): ValidationIssue[] => {
    ...
    // 4. Shopping Settings (Merchant Center ID and WebsiteTraffic Country)
    ...
    const trimmedWebsiteTrafficCountry = (WebsiteTrafficCountry || feedLabel || "").trim();
    if (!trimmedWebsiteTrafficCountry) {
      issues.push({
        id: "camp-WebsiteTraffic-country-req",
        level: "Campaign",
        parameter: "WebsiteTraffic Country",
        message: "WebsiteTraffic Country is required.",
        step: "AD_GROUP",
        settingKey: "product_groups"
      });
    }
                  ...
                  <div className="space-y-4 pt-2 text-xs">
                    ...
                    <div className="p-4 border border-blue-500/30 bg-blue-500/5 rounded-xl space-y-3">
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 pt-1">
                        ...
                        <div className="space-y-1">
                          <label className="text-slate-700 font-semibold text-xs">Feed / WebsiteTraffic Country <span className="text-rose-500">*</span></label>
                          <select
                            value={WebsiteTrafficCountry}
                            onChange={(e) => {
                              setWebsiteTrafficCountry(e.target.value);
              ...
              <div className="p-6 rounded-2xl border border-slate-200 bg-white space-y-4 shadow-sm">
                ...
                <div className="grid grid-cols-2 gap-y-4 text-xs">
                  ...
                  <div>
                    ...
                    <p className="text-slate-800 font-semibold mt-1">WebsiteTraffic</p>
                  </div>
                  <div>
                    <p className="text-slate-500 font-medium">Merchant Center and CSS</p>
                    <p className="text-slate-800 font-semibold mt-1">{merchantCenterId || "Not set"} ({WebsiteTrafficCountry}) / CSS: Google Shopping</p>
                  </div>
                  <div>
                ...
                onClick={async () => {
                  ...
                  try {
                    ...
                    const sanitizedShoppingSetting = {
                      ...
                      WebsiteTrafficCountry: WebsiteTrafficCountry.trim(),
                      feedLabel: feedLabel.trim() || WebsiteTrafficCountry.trim(),
                    ...
                    if (!payloadToLaunch.shoppingSetting || !payloadToLaunch.shoppingSetting.merchantId || !payloadToLaunch.shoppingSetting.WebsiteTrafficCountry) {
                      setSubmitError("Shopping Setting is incomplete. Merchant Center ID and WebsiteTraffic Country are required.");
                      setIsPublishing(false);
                      return;
                    }

                    const res = await fetch(`${BACKEND}/api/ads/campaigns/website-traffic/shopping`, {
                      method: "POST",
