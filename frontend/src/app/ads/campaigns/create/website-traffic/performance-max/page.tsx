...
function WebsiteTrafficPerformanceMaxContent() {
    ...
    try {
      ...
      await fetch(`${BACKEND}/api/ads/campaign/draft`, {
        ...
        body: JSON.stringify({
          ...
          campaignName: campaignName || "WebsiteTraffic-Performance Max-1",
  ...
  const [lfWebsiteTrafficcoringQuestion, setLfWebsiteTrafficcoringQuestion] = useState<string>("");
  ...
  useEffect(() => {
    ...
    if (draftId) {
      ...
      try {
        ...
        console.warn("Could not parse googleAds_prefill_campaign for Website Traffic PMax:", err);
      }
    }
  }, [customerId, draftId]);
                ...
                <div className="p-6 rounded-2xl border border-slate-200 bg-white space-y-3 shadow-sm animate-in fade-in duration-150">
                  ...
                  <div className="space-y-3 pt-1">
                    ...
                    <label className="flex items-center gap-3 cursor-pointer">
                      <input type="radio" name="euPolWebsiteTraffic" checked={euPoliticalAds === "YES"} onChange={() => setEuPoliticalAds("YES")} className="text-primary h-4 w-4" />
                      <span>Yes, this campaign has EU political ads</span>
                    </label>
                    <label className="flex items-center gap-3 cursor-pointer">
                      <input type="radio" name="euPolWebsiteTraffic" checked={euPoliticalAds === "NO"} onChange={() => setEuPoliticalAds("NO")} className="text-primary h-4 w-4" />
                        ...
                        <div className="flex items-center justify-between">
                          ...
                          <div className="flex items-center gap-2">
                            <button
                              type="button"
                              onClick={() => {
                                const aiGens = Array.from({ length: Math.max(headlines.length, 3) }, (_, i) => `AI Ad Headline ${i + 1}`);
                                setHeadlines(aiGens);
                              }}
              ...
              {(() => {
                ...
                if (!businessName || !businessName.trim()) {
                  issues.push({
                    ...
                    message: "Business Name is required for Performance Max campaigns. Please enter your business name in the Asset Group section.",
                    onFix: () => {
                ...
                if (validH.length < 3) {
                  ...
                  const tooLongH = validH.filter(h => h.trim().length > 30);
                  if (tooLongH.length > 0) {
                    issues.push({
                      id: "headlinesTooLong",
                      parameter: "Headlines",
                      message: `${tooLongH.length} headline(s) exceed 30 characters: "${tooLongH[0].trim().slice(0, 35)}…". Each headline must be 30 characters or fewer.`,
                      onFix: () => {
                        setWizardStep("ASSET_GROUP");
                        setIsAssetsSectionOpen(true);
                      }
                ...
                if (validLH.length < 1) {
                  ...
                  const tooLongLH = validLH.filter(lh => lh.trim().length > 90);
                  if (tooLongLH.length > 0) {
                    issues.push({
                      id: "longHeadlinesTooLong",
                      parameter: "Long headlines",
                      message: `${tooLongLH.length} long headline(s) exceed 90 characters. Each long headline must be 90 characters or fewer.`,
                      onFix: () => {
                        setWizardStep("ASSET_GROUP");
                        setIsAssetsSectionOpen(true);
                      }
                ...
                if (validD.length < 2) {
                  ...
                  const tooLongD = validD.filter(d => d.trim().length > 90);
                  if (tooLongD.length > 0) {
                    issues.push({
                      id: "descriptionsTooLong",
                      parameter: "Descriptions",
                      message: `${tooLongD.length} description(s) exceed 90 characters. Each description must be 90 characters or fewer.`,
                      onFix: () => {
                        setWizardStep("ASSET_GROUP");
                        setIsAssetsSectionOpen(true);
                      }
              ...
              <div className="p-6 rounded-2xl border border-slate-200 bg-white space-y-4 shadow-sm">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div className="md:col-span-2">
                    ...
                    <span className="font-bold text-slate-900">WebsiteTraffic (Downloads, Phone call WebsiteTraffic)</span>
                  </div>
                </div>
              </div>
          ...
          {wizardStep !== "SUMMARY" ? (
            ...
            <button
              ...
              onClick={async () => {
                ...
                const tooLongHeadlines = validHeadlines.filter(h => h.trim().length > 30);
                if (tooLongHeadlines.length > 0) {
                  setSubmitError(`Headline too long: "${tooLongHeadlines[0].trim()}" (${tooLongHeadlines[0].trim().length} chars). Each headline must be 30 characters or fewer. Please shorten it in the Asset Group.`);
                  setWizardStep("ASSET_GROUP");
                  setIsAssetsSectionOpen(true);
                  return;
                }
                ...
                const tooLongLongHeadlines = validLongHeadlines.filter(lh => lh.trim().length > 90);
                if (tooLongLongHeadlines.length > 0) {
                  setSubmitError(`Long headline too long: "${tooLongLongHeadlines[0].trim().slice(0, 50)}…" (${tooLongLongHeadlines[0].trim().length} chars). Each long headline must be 90 characters or fewer.`);
                  setWizardStep("ASSET_GROUP");
                  setIsAssetsSectionOpen(true);
                  return;
                }
                ...
                const tooLongDescriptions = validDescriptions.filter(d => d.trim().length > 90);
                if (tooLongDescriptions.length > 0) {
                  setSubmitError(`Description too long: "${tooLongDescriptions[0].trim().slice(0, 50)}…" (${tooLongDescriptions[0].trim().length} chars). Each description must be 90 characters or fewer.`);
                  setWizardStep("ASSET_GROUP");
                  setIsAssetsSectionOpen(true);
                  return;
                }
                ...
                // Business Name — required by Google Performance Max backend
                if (!businessName || !businessName.trim()) {
                  setSubmitError("Business Name is required. Please enter your business name in the Asset Group section (Brand Guidelines).");
                  setWizardStep("ASSET_GROUP");
                  setIsAssetGroupInfoOpen(true);
                  return;
                }
                ...
                try {
                  ...
                  const targetRoasNum = biddingFocus === "Target ROAS" && targetRoasValue ? Number(targetRoasValue) / 100 : undefined;
                  ...
                  if (!res.ok) {
                    throw new Error(data.error || "Failed to publish WebsiteTraffic Performance Max campaign.");
                  }
            ...
            <div className="space-y-5">
                  ...
                  <div className="space-y-2">
                    ...
                    >
                      ...
                      <optgroup label="Business">
                        ...
                        <option value="What is your annual WebsiteTraffic volume?">What is your annual WebsiteTraffic volume?</option>
              ...
              <div className="space-y-3 pt-3 border-t border-slate-200">
                ...
                <div className="p-3 bg-slate-50 border border-slate-200 rounded-xl text-[11px] text-slate-700 space-y-1">
                  <span className="font-semibold block">Download collected WebsiteTraffic in ad extensions table</span>
                  <p className="text-slate-500">You can only download WebsiteTraffic that've been collected within the last 30 days</p>
                </div>
              ...
              <div className="space-y-3 pt-3 border-t border-slate-200">
                <div>
                  ...
                  <p className="text-[11px] text-slate-500">Optimize for WebsiteTraffic with higher intent or for more WebsiteTraffic overall. <a href="https://support.google.com/google-ads/answer/10089406?hl=en_US#optimization" target="_blank" rel="noreferrer" className="text-primary hover:underline font-semibold">Learn more</a></p>
                </div>
                ...
                <div className="grid grid-cols-2 gap-3">
                  <label className={`p-3 rounded-xl border flex flex-col justify-between cursor-pointer transition-all ${lfFormType === "MORE_VOLUME" ? "border-primary bg-primary/10" : "border-slate-200 bg-slate-50 hover:bg-white"}`}>
                    ...
                    <p className="text-[10px] text-slate-500 mt-1">This may result in WebsiteTraffic with lower intent.</p>
                  </label>
                  ...
                  <label className={`p-3 rounded-xl border flex flex-col justify-between cursor-pointer transition-all ${lfFormType === "MORE_QUALIFIED" ? "border-primary bg-primary/10" : "border-slate-200 bg-slate-50 hover:bg-white"}`}>
                    ...
                    <p className="text-[10px] text-slate-500 mt-1">This may result in fewer WebsiteTraffic or a higher cost per lead.</p>
                  </label>
                </div>
              ...
              <div className="space-y-4 animate-in slide-in-from-right-3 duration-200">
                <div className="space-y-1">
                  ...
                  >
                    ...
                    {["All customers", "Purchasers", "High-value customers", "Disengaged customers", "Qualified WebsiteTraffic", "Converted WebsiteTraffic", "Paid subscribers", "Cart abandoners", "Loyalty program members", "Tier 1", "Tier 2", "Tier 3", "Tier 4", "Tier 5", "Tier 6", "Tier 7"].map(opt => (
