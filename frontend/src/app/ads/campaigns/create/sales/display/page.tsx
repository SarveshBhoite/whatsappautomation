...
export default function SalesDisplayPage() {
  ...
  const [displayCampaignName, setDisplayCampaignName] = useState<string>(`Sales-Display-${Date.now().toString().slice(-4)}`);
  ...
  useEffect(() => {
    ...
    try {
      if (typeof window !== "undefined") {
        ...
        if (prefillRaw) {
          ...
          if (Array.isArray(prefill.images) && prefill.images.length > 0) {
            ...
            if (imgUrls.length > 0) {
              setLandscapeImagesList(prev => Array.from(new Set([...prev, ...imgUrls])));
              setSquareImagesList(prev => Array.from(new Set([...prev, ...imgUrls])));
            }
          }
      ...
      console.warn("Could not parse googleAds_prefill_campaign for Sales Display:", err);
    }
  }, [customerId]);
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                          <div className="space-y-1.5">
                            ...
                            <input
                              ...
                              value={startDate || todayFormattedStr}
                              onChange={(e) => {
                                const newStart = e.target.value || todayFormattedStr;
                            ...
                            />
                            <span className="text-[10px] text-slate-500">Must be today or a future date (defaults to today)</span>
                          </div>
                          <div className="space-y-1.5">
                  ...
                  {openAdSetting === "assets" ? (
                    <>
                      ...
                      <div className="space-y-4 pt-2 text-xs">
                        ...
                        <div className="p-4 rounded-xl border border-slate-200 bg-slate-50 space-y-3">
                          ...
                          <p className="text-[11px] text-slate-500">Add up to 15 images. At least 1 landscape (1.91:1) and 1 square (1:1) image required <span className="text-rose-500 font-bold">*</span>.</p>

                          {(landscapeImagesList.length > 0 || squareImagesList.length > 0) && (
                            <div className="space-y-3 pt-2">
                              {landscapeImagesList.length > 0 && (
                                <div className="space-y-1.5">
                                  <span className="text-[11px] font-semibold text-slate-600">Landscape (1.91:1) - {landscapeImagesList.length}</span>
                                    ...
                                    {landscapeImagesList.map((imgUrl, i) => (
                                      <div key={i} className="relative group p-2 rounded-xl bg-white border border-slate-200 space-y-2">
                                        <div className="h-24 w-full rounded-lg overflow-hidden bg-slate-50 border border-slate-200 flex items-center justify-center">
                                          <img src={imgUrl} alt={`Landscape ${i + 1}`} className="h-full w-full object-cover" />
                                        </div>
                                        <div className="flex items-center justify-between gap-1">
                                          <span className="text-[10px] font-mono text-emerald-600 truncate flex-1" title={imgUrl}>
                                            {imgUrl.startsWith("data:") ? `Land_${i + 1}.jpg` : imgUrl}
                                          </span>
                                          <button
                                            type="button"
                              ...
                              {squareImagesList.length > 0 && (
                                <div className="space-y-1.5">
                                  <span className="text-[11px] font-semibold text-slate-600">Square (1:1) - {squareImagesList.length}</span>
                                  <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3">
                                    {squareImagesList.map((imgUrl, i) => (
                                      <div key={i} className="relative group p-2 rounded-xl bg-white border border-slate-200 space-y-2">
                                        <div className="h-24 w-full rounded-lg overflow-hidden bg-slate-50 border border-slate-200 flex items-center justify-center">
                                          <img src={imgUrl} alt={`Square ${i + 1}`} className="h-full w-full object-cover" />
                                        </div>
                                        <div className="flex items-center justify-between gap-1">
                                          <span className="text-[10px] font-mono text-cyan-600 truncate flex-1" title={imgUrl}>
                                            {imgUrl.startsWith("data:") ? `Square_${i + 1}.jpg` : imgUrl}
                                          </span>
                                          <button
                                            type="button"
                    ...
                    >
                      <div className="flex items-center gap-16">
                        ...
                        <div className="text-[11px] text-slate-500">
                          {landscapeImagesList.length + squareImagesList.length} Image(s), {logosList.length} Logo(s), {videosList.length} Video(s)
                        </div>
                      </div>
                      <button
                        type="button"
              ...
              onClick={async () => {
                ...
                try {
                  ...
                  const payloadToLaunch = {
                    ...
                    campaignName: displayCampaignName.trim() || `Sales-Display-${Date.now()}`,
                    channelType: "DISPLAY",
                    ...
                    images: formattedImages.length > 0 ? formattedImages : [
                      { data: "https://ik.imagekit.io/automationjds/tr:w-1200,h-628,cm-pad_resize,bg-FFFFFF/gads_dg_image_1787574968684_aimaths_YX-Kb7zvI.jpg", fieldType: "MARKETING_IMAGE" },
                      { data: "https://ik.imagekit.io/automationjds/tr:w-1200,h-1200,cm-pad_resize,bg-FFFFFF/gads_dg_image_1787574968684_aimaths_YX-Kb7zvI.jpg", fieldType: "SQUARE_MARKETING_IMAGE" }
                    ],
                    logos: logosList.length > 0 ? logosList.map(data => (typeof data === "string" ? { data, fieldType: "LOGO" } : data)) : [
                      { data: "https://ik.imagekit.io/automationjds/tr:w-1200,h-1200,cm-pad_resize,bg-FFFFFF/gads_dg_logo_1787574973938_google_ads_logo_FJndWjppS.jpg", fieldType: "LOGO" }
                    ],
                  ...
                  const res = await fetch(`${BACKEND}/api/ads/campaigns/sales/display`, {
                    method: "POST",
