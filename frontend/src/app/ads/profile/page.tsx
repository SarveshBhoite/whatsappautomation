...
const PROFILE_TABS_SEQUENCE: { key: TabKey; label: string; shortLabel: string }[] = [
  ...
  { key: "products_services", label: "Products & Services", shortLabel: "Products & Services" },
  { key: "target_audience", label: "Target Audience & Personas", shortLabel: "Target Audience" },
  { key: "locations", label: "Locations & Service Areas", shortLabel: "Locations" },
  { key: "conversion_goals", label: "Conversion Goals", shortLabel: "Conversion Goals" },
  { key: "brand_profile", label: "Brand Profile", shortLabel: "Brand Profile" },
  { key: "competitors", label: "Competitor Intelligence", shortLabel: "Competitors" },
  { key: "seo_keywords", label: "SEO & Keywords", shortLabel: "SEO & Keywords" },
...
function ProfilePageContent() {
  ...
  const [orgId, setOrgId] = useState<string>("demo-org-123");
        ...
        if (isMounted) {
          ...
          const loadedGoals: ConversionGoalItem[] = Array.isArray(data.conversionGoals)
            ? data.conversionGoals.map((g: any, idx: number): ConversionGoalItem => ({
                ...
                currency: g.currency || defaultCurr,
  ...
  const handleSaveProfile = async (approve = false) => {
    ...
    try {
      ...
      setSaveSuccessMsg(
        approve
          ? "Profile approved! Business intelligence is active for AI Guided Campaign Creation."
  ...
  const handleSaveAndNext = async (nextTab: TabKey) => {
  ...
  const currentTabIndex = PROFILE_TABS_SEQUENCE.findIndex((t) => t.key === activeTab);
  const prevTabConfig = currentTabIndex > 0 ? PROFILE_TABS_SEQUENCE[currentTabIndex - 1] : null;
  const nextTabConfig = currentTabIndex < PROFILE_TABS_SEQUENCE.length - 1 ? PROFILE_TABS_SEQUENCE[currentTabIndex + 1] : null;

  const renderSectionFooterNav = (overrideNextTab?: TabKey, overridePrevTab?: TabKey) => {
    const prev = overridePrevTab ? PROFILE_TABS_SEQUENCE.find(t => t.key === overridePrevTab) : prevTabConfig;
    const next = overrideNextTab ? PROFILE_TABS_SEQUENCE.find(t => t.key === overrideNextTab) : nextTabConfig;

    return (
      <ProfileSectionFooterNav
        currentTab={activeTab}
        stepNumber={currentTabIndex + 1}
        totalSteps={PROFILE_TABS_SEQUENCE.length}
        nextSectionName={next?.shortLabel}
        prevSectionName={prev?.shortLabel}
        onSaveAndNext={async () => {
          if (next) await handleSaveAndNext(next.key);
        }}
        onPrevious={prev ? () => {
  ...
  const returnToAdsUrl = `/ads${customerId ? `?customerId=${customerId}` : ""}`;

  if (!customerId) {
    return (
      <div className="flex-1 h-full min-h-0 overflow-y-auto bg-slate-50 flex items-center justify-center p-4">
        <div className="max-w-md w-full p-6 rounded-3xl bg-white border border-slate-200 text-center space-y-4 shadow-sm">
          <div className="w-12 h-12 rounded-2xl bg-amber-50 text-amber-600 flex items-center justify-center mx-auto">
            <AlertCircle className="w-6 h-6" />
          </div>
          <h2 className="text-base font-bold text-slate-900">No Customer Account Selected</h2>
          <p className="text-xs text-slate-500">
            A Google Ads Customer ID is required to access the customer-scoped Business &amp; Marketing Profile.
          </p>
          <button
            onClick={() => router.push("/ads")}
            className="w-full py-2.5 rounded-xl bg-blue-600 hover:bg-blue-700 text-white font-bold text-xs transition-all shadow-sm cursor-pointer"
          >
            Return to Google Ads Dashboard
          </button>
        </div>
      </div>
    );
  ...
  return (
    ...
    >
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-6 space-y-6">
        ...
        {error && (
          <div className="p-4 rounded-2xl bg-rose-50 border border-rose-200 text-rose-800 text-xs flex items-center justify-between gap-2 font-semibold">
            <div className="flex items-center gap-2">
        ...
        {loading ? (
          ...
          <>
            ...
            <div className="flex flex-col lg:flex-row items-start gap-6">
              ...
              <div className="animate-fadeIn space-y-6 transition-all duration-300">
                  ...
                  <div className="flex items-center justify-between pb-2 border-b border-slate-100">
                    <div>
                      <div className="flex items-center gap-2">
                        ...
                        <h2 className="text-sm font-bold text-slate-900">Business Information</h2>
                      </div>
                      <p className="text-xs text-slate-500 mt-0.5">
                        Customer-scoped legal identity, industry category, business model, and operational contact details.
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                    <div className="space-y-1.5">
                      ...
                      <input
                        ...
                        className="w-full px-3.5 py-2.5 text-xs rounded-xl border border-slate-200 focus:outline-none focus:border-blue-500 font-semibold"
                      />
                    </div>
                    ...
                    <div className="space-y-1.5">
                      ...
                      <select
                        ...
                        className="w-full px-3.5 py-2.5 text-xs rounded-xl border border-slate-200 focus:outline-none focus:border-blue-500 font-semibold bg-white"
                      >
                    ...
                    <div className="space-y-1.5">
                      ...
                      <input
                        ...
                        className="w-full px-3.5 py-2.5 text-xs rounded-xl border border-slate-200 focus:outline-none focus:border-blue-500 font-mono font-semibold"
                      />
                    </div>
                    ...
                    <div className="space-y-1.5 sm:col-span-2">
                      ...
                      <textarea
                        ...
                        className="w-full px-3.5 py-2.5 text-xs rounded-xl border border-slate-200 focus:outline-none focus:border-blue-500 leading-relaxed font-medium"
                      />
                    </div>
                  </div>
                    ...
                    <div className="space-y-2 p-5 rounded-2xl bg-slate-50/80 border border-slate-200">
                      ...
                      <div className="flex flex-wrap gap-1.5 min-h-[36px]">
                        {serviceAreas.map((area, idx) => (
                          <span
                            key={idx}
                            className="inline-flex items-center gap-1.5 px-2.5 py-1 text-[11px] font-semibold rounded-lg bg-white border border-slate-200 text-slate-800 shadow-2xs"
                          >
                            {area}
                            <button
                              type="button"
                      ...
                      <div className="flex gap-1.5 pt-1">
                        <input
                          ...
                          placeholder="Add area (e.g. Mumbai, Maharashtra)..."
                ...
                <div className="p-6 rounded-3xl bg-white border border-slate-200 shadow-sm space-y-6">
                  <div className="flex items-center justify-between pb-2 border-b border-slate-100">
                    <div>
                      <div className="flex items-center gap-2">
                        ...
                        <h2 className="text-sm font-bold text-slate-900">Brand &amp; Market Intelligence</h2>
                      </div>
                      <p className="text-xs text-slate-500 mt-0.5">
                        Core business offerings, audience persona, and marketing details used to prefill AI Guided campaigns.
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="space-y-1.5">
                    ...
                    <textarea
                      ...
                      placeholder="Concise 1-2 sentence overview of your brand and what makes your products/services unique..."
                      className="w-full px-3.5 py-2.5 text-xs rounded-xl border border-slate-200 focus:outline-none focus:border-blue-500 leading-relaxed font-medium"
                    />
                  </div>
                  ...
                  <div className="space-y-1.5">
                    <label className="text-xs font-bold text-slate-700 flex items-center justify-between">
                      <span>Target Audience &amp; Ideal Customers</span>
                      <button
                        type="button"
                    ...
                    <textarea
                      ...
                      placeholder="e.g. Small business owners, marketing managers, and regional enterprises looking for leads..."
                      className="w-full px-3.5 py-2.5 text-xs rounded-xl border border-slate-200 focus:outline-none focus:border-blue-500 font-medium"
                    />
                  </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-5 pt-2">
                  ...
                  <div className="space-y-2 p-5 rounded-2xl bg-slate-50/80 border border-slate-200">
                    ...
                    <div className="flex gap-1.5 pt-1">
                      <input
                        ...
                        placeholder="Add quick product (press Enter)..."
                        className="flex-1 px-3 py-1.5 text-xs rounded-xl border border-slate-200 bg-white"
                      />
                      <button
                        type="button"
                  ...
                  <div className="space-y-2 p-5 rounded-2xl bg-slate-50/80 border border-slate-200">
                    ...
                    <div className="flex gap-1.5 pt-1">
                      <input
                        ...
                        placeholder="Add quick service (press Enter)..."
                        className="flex-1 px-3 py-1.5 text-xs rounded-xl border border-slate-200 bg-white"
                      />
                      <button
                        type="button"
                  ...
                  <div className="space-y-2 p-5 rounded-2xl bg-slate-50/80 border border-slate-200">
                    <label className="text-xs font-bold text-slate-800 flex items-center justify-between">
                      <span>Key Offerings &amp; Guarantees ({keyOfferings.length})</span>
                    </label>
                    <div className="flex flex-wrap gap-1.5 min-h-[36px]">
                    ...
                    <div className="flex gap-1.5 pt-1">
                      <input
                        ...
                        placeholder="Add USP or guarantee..."
                        className="flex-1 px-3 py-1.5 text-xs rounded-xl border border-slate-200 bg-white"
                      />
                      <button
                        type="button"
                  ...
                  <div className="space-y-2 p-5 rounded-2xl bg-slate-50/80 border border-slate-200">
                    <label className="text-xs font-bold text-slate-800 flex items-center justify-between">
                      ...
                      <button
                        ...
                        className="text-blue-600 hover:text-blue-700 text-[11px] font-semibold flex items-center gap-1 cursor-pointer"
                      >
                        <span>Manage Locations Master ({locationRecords.length})</span> &rarr;
                      </button>
                    </label>
                    <div className="flex flex-wrap gap-1.5 min-h-[36px]">
            ...
            {/* TAB CONTENT: PRODUCTS & SERVICES MASTER */}
                  ...
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-4 border-b border-slate-100">
                    <div className="space-y-1">
                      <div className="flex items-center gap-2">
                        ...
                        <h2 className="text-sm font-bold text-slate-900">Products &amp; Services Master</h2>
                      </div>
                      <p className="text-xs text-slate-500">
                    ...
                    <div className="flex items-center gap-2.5">
                      {psSubTab === "products" && products.length > 0 && (
                        <button
                          ...
                          className="px-3 py-2 rounded-xl bg-slate-100 hover:bg-rose-50 text-slate-600 hover:text-rose-600 border border-slate-200 hover:border-rose-200 text-xs font-bold transition-all cursor-pointer inline-flex items-center gap-1.5"
                      ...
                      {psSubTab === "services" && services.length > 0 && (
                        <button
                          type="button"
                          onClick={() => {
                            if (window.confirm("Are you sure you want to remove all services?")) {
                              setServices([]);
                            }
                          }}
                          className="px-3 py-2 rounded-xl bg-slate-100 hover:bg-rose-50 text-slate-600 hover:text-rose-600 border border-slate-200 hover:border-rose-200 text-xs font-bold transition-all cursor-pointer inline-flex items-center gap-1.5"
                      ...
                      {psSubTab === "products" ? (
                        <button
                          ...
                          className="px-4 py-2 rounded-xl bg-blue-600 hover:bg-blue-700 text-white text-xs font-bold transition-all cursor-pointer shadow-sm shadow-blue-500/20 inline-flex items-center gap-1.5"
                        >
                          <Plus className="w-3.5 h-3.5" />
                    ...
                    <div className="py-14 text-center border border-dashed border-slate-200 rounded-2xl p-6 bg-slate-50/50 space-y-3">
                      ...
                      <div className="space-y-1">
                        ...
                        <p className="text-[11px] text-slate-500 max-w-sm mx-auto">
                          Add your physical or digital products with pricing, features, benefits, and URLs to power automated campaigns.
                        </p>
                      </div>
                      <button
                        type="button"
                        onClick={openAddProductModal}
                        className="px-4 py-2 rounded-xl bg-blue-600 hover:bg-blue-700 text-white text-xs font-bold inline-flex items-center gap-1.5 cursor-pointer shadow-xs"
                      >
                        <Plus className="w-3.5 h-3.5" />
                      ...
                      {products.map((prod) => (
                        ...
                        >
                          <div className="flex items-start justify-between gap-3 mb-2">
                            <div className="space-y-1 min-w-0">
                              <div className="flex items-center gap-2 flex-wrap">
                                <h3 className="text-xs font-bold text-slate-900 truncate">{prod.name}</h3>
                                {prod.category && (
                                ...
                                <div className="flex items-center gap-2 text-[11px] text-slate-600 flex-wrap">
                                  ...
                                  {prod.productUrl && (
                                    <a
                                      ...
                                      target="_blank"
                                      rel="noopener noreferrer"
                                      className="text-blue-600 hover:underline inline-flex items-center gap-1 truncate max-w-[180px]"
                                    >
                          ...
                          {(prod.usp || prod.targetAudience) && (
                            <div className="space-y-1 mb-3 text-[11px] p-2.5 rounded-xl bg-slate-50 border border-slate-150">
                              {prod.usp && (
                  ...
                  {psSubTab === "services" && services.length === 0 && (
                    <div className="py-14 text-center border border-dashed border-slate-200 rounded-2xl p-6 bg-slate-50/50 space-y-3">
                      ...
                      <div className="space-y-1">
                        ...
                        <p className="text-[11px] text-slate-500 max-w-sm mx-auto">
                          Add your consulting, professional, or digital services with pricing, features, benefits, and URLs to power campaigns.
                        </p>
                      </div>
                      <button
                        type="button"
                      ...
                      {services.map((serv) => (
                        ...
                        >
                          <div className="flex items-start justify-between gap-3 mb-2">
                            <div className="space-y-1 min-w-0">
                              <div className="flex items-center gap-2 flex-wrap">
                                <h3 className="text-xs font-bold text-slate-900 truncate">{serv.name}</h3>
                                {serv.category && (
                                  <span className="px-2 py-0.5 text-[10px] font-bold rounded-md bg-slate-100 text-slate-700 border border-slate-200">
                                    {serv.category}
                                  </span>
              ...
              <div className="animate-fadeIn space-y-6 transition-all duration-300">
                ...
                <div className="flex items-center justify-between flex-wrap gap-4 pb-2 border-b border-slate-100">
                  <div className="space-y-1">
                    <div className="flex items-center gap-2">
                      ...
                      <h2 className="text-sm font-bold text-slate-900">Locations &amp; Service Areas Master</h2>
                    </div>
                    <p className="text-xs text-slate-500">
                      Customer-scoped business locations, branches, retail stores, and service areas to inform campaign delivery and coverage.
                    </p>
                  </div>

                  <div className="flex items-center gap-3">
                    <span className="text-xs font-bold px-3 py-1 rounded-full bg-slate-100 text-slate-700 border border-slate-200">
                      {locationRecords.length} Total ({locationRecords.filter((l) => l.isActive).length} Active)
                    </span>
                    <button
                      type="button"
                ...
                {locationRecords.length === 0 ? (
                  <div className="p-12 text-center border-2 border-dashed border-slate-200 rounded-3xl space-y-3">
                    ...
                    <h3 className="text-sm font-bold text-slate-800">No Locations Configured</h3>
                    <p className="text-xs text-slate-500 max-w-sm mx-auto">
                      Add your headquarters, branch offices, stores, or regional service areas to build reusable location profiles.
                    </p>
                    <button
                      type="button"
                      onClick={openAddLocationModal}
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {locationRecords.map((loc) => {
                      ...
                      return (
                        ...
                        >
                          {/* Card Top: Type, Status, Actions */}
                          <div className="flex items-center justify-between gap-2 flex-wrap">
                            <div className="flex items-center gap-2">
                              <span className={`px-2.5 py-0.5 rounded-full text-[10px] font-extrabold border ${typeBadgeStyles}`}>
                                {loc.locationType}
                              </span>
                              <span
                            ...
                            <div className="flex items-center gap-1">
                              ...
                              <button
                                ...
                                className="p-1.5 rounded-lg border border-slate-200 text-slate-600 hover:bg-slate-100 hover:text-blue-600 transition-colors cursor-pointer"
                                title="Edit Location"
                              >
                                <Edit3 className="w-3.5 h-3.5" />
                              </button>
                          ...
                          <div>
                            <h3 className="text-sm font-bold text-slate-900 leading-snug">{loc.locationName}</h3>
                            <div className="flex items-center gap-1.5 text-xs text-slate-600 mt-1">
                              ...
                              {loc.pincode && (
                                <span className="text-slate-400 font-mono text-[11px]">({loc.pincode})</span>
                              )}
                            </div>
                            {loc.areaLocality && (
                              <p className="text-[11px] text-slate-500 font-medium mt-0.5 pl-5">
                                Area / Locality: {loc.areaLocality}
                              </p>
                            )}
                          </div>
                          ...
                          {/* Coordinates and Radius Bar */}
                          {((loc.latitude && loc.longitude) || loc.radius) && (
                            <div className="flex items-center gap-2 flex-wrap text-[11px] text-slate-600">
                              {loc.latitude && loc.longitude && (
                                <span className="px-2 py-1 rounded-lg bg-slate-100 font-mono text-[10px]">
                                  Lat: {loc.latitude}, Lng: {loc.longitude}
                                </span>
                              )}
                              {loc.radius && (
                                <span className="px-2 py-1 rounded-lg bg-blue-50 text-blue-800 border border-blue-200 font-semibold text-[10px]">
                                  Radius: {loc.radius}
                                </span>
                              )}
                            </div>
                  ...
                  <div className="px-6 py-4 border-b border-slate-100 flex items-center justify-between gap-3">
                    <div className="flex items-center gap-2.5">
                      ...
                      <div>
                        ...
                        <p className="text-[11px] text-slate-500">
                          Configure customer business location or service area coverage details.
                        </p>
                      </div>
                    </div>
                    <button
                  ...
                  <div className="p-6 overflow-y-auto space-y-4 text-xs">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                      <div className="sm:col-span-1 space-y-1">
                        ...
                        <input
                          ...
                          placeholder="e.g. Pune Tech Hub, South Delhi Store"
                          className="w-full px-3 py-2 text-xs rounded-xl border border-slate-200 focus:outline-none focus:border-blue-500 font-semibold"
                        />
                      </div>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                      ...
                      <div className="space-y-1">
                        ...
                        <input
                          ...
                          placeholder="e.g. Maharashtra, Texas"
                          className="w-full px-3 py-2 text-xs rounded-xl border border-slate-200 focus:outline-none focus:border-blue-500"
                        />
                      </div>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                      <div className="space-y-1">
                        ...
                        <input
                          ...
                          placeholder="e.g. Baner, Bandra West, Downtown"
                          className="w-full px-3 py-2 text-xs rounded-xl border border-slate-200 focus:outline-none focus:border-blue-500"
                        />
                      </div>

                      <div className="space-y-1">
                        <label className="text-xs font-bold text-slate-700">Pincode / Postal Code</label>
                        <input
                          type="text"
                          value={locPincode}
                          onChange={(e) => setLocPincode(e.target.value)}
                          placeholder="e.g. 411045, 78701"
                    ...
                    <div className="space-y-1">
                      ...
                      <textarea
                        ...
                        placeholder="Complete street address, building/floor, landmark..."
                    ...
                    {/* Row 6: Languages */}
                    <div className="space-y-1.5">
                      <label className="text-xs font-bold text-slate-700">Languages Served</label>
                      <div className="flex flex-wrap gap-1 min-h-[32px] p-2 rounded-xl bg-slate-50 border border-slate-200">
                        {locLanguages.map((lng, idx) => (
                          <span key={idx} className="inline-flex items-center gap-1 px-2.5 py-1 rounded-md bg-white border border-slate-200 text-xs font-semibold">
                            <span>{lng}</span>
                            <button
                              type="button"
                      ...
                      <div className="flex gap-1.5">
                        ...
                        <button
                          ...
                          className="px-3.5 py-1.5 text-xs font-bold rounded-xl bg-blue-600 text-white cursor-pointer"
                        >
                          Add
                        </button>
                      </div>
                    ...
                    <div className="space-y-1">
                      ...
                      <textarea
                        ...
                        placeholder="Operating hours, parking availability, regional marketing nuances..."
                        className="w-full px-3 py-2 text-xs rounded-xl border border-slate-200 focus:outline-none focus:border-blue-500"
                      />
                    </div>
                  </div>
                ...
                <div className="p-6 rounded-3xl bg-white border border-slate-200 shadow-sm space-y-6">
                  ...
                  <div className="flex items-center justify-between flex-wrap gap-4">
                    <div>
                      <div className="flex items-center gap-2">
                        ...
                        <h2 className="text-sm font-bold text-slate-900">Conversion Goals Master</h2>
                      </div>
                      <p className="text-xs text-slate-500 mt-1">
                        Define customer-scoped reusable conversion goals (e.g. Website Purchase, Lead Form, Phone Call, WhatsApp, Bookings) to configure Google Ads conversion tracking and bidding.
                      </p>
                    </div>
                  ...
                  {conversionGoals.length === 0 ? (
                    <div className="p-10 rounded-2xl border border-dashed border-slate-200 text-center space-y-3">
                      ...
                      <h3 className="text-xs font-bold text-slate-700">No Conversion Goals Added Yet</h3>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                      {conversionGoals.map((goal) => (
                        ...
                        >
                          {/* Card Header: Type Badge & Primary Pill & Status & Actions */}
                          <div className="flex items-start justify-between gap-2">
                            <div className="flex items-center gap-1.5 flex-wrap">
                              <span
                          ...
                          {/* Goal Name & Source */}
                          <div>
                            <h3 className="text-sm font-bold text-slate-900 leading-snug">{goal.goalName}</h3>
                            <div className="flex items-center gap-1.5 text-xs text-slate-500 mt-1">
                              ...
                              {(goal.conversionValue !== undefined && goal.conversionValue !== null && String(goal.conversionValue).trim() !== "") && (
                                <span className="ml-auto font-mono text-[11px] font-bold text-emerald-700 bg-emerald-50 px-2 py-0.5 rounded-md border border-emerald-200/60">
            ...
            {isGoalModalOpen && (
              <div className="fixed inset-0 z-50 bg-slate-900/40 backdrop-blur-xs flex items-center justify-center p-4">
                <div className="max-w-xl w-full bg-white rounded-3xl border border-slate-200 shadow-2xl overflow-hidden flex flex-col max-h-[90vh] animate-fadeIn">
                  {/* Modal Header */}
                  <div className="px-6 py-4 border-b border-slate-100 flex items-center justify-between gap-3">
                    <div className="flex items-center gap-2.5">
                      <div className="w-8 h-8 rounded-xl bg-blue-100 text-blue-700 flex items-center justify-center">
                      ...
                      <div>
                        ...
                        <p className="text-[11px] text-slate-500">
                          Configure customer reusable conversion goal and bidding attribution properties.
                        </p>
                      </div>
                    </div>
                    <button
                  ...
                  <div className="p-6 overflow-y-auto space-y-4 text-xs">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                      <div className="sm:col-span-2 space-y-1">
                        <label className="text-xs font-bold text-slate-700">Conversion Goal Name *</label>
                        <input
                          type="text"
                          value={goalName}
                          onChange={(e) => setGoalName(e.target.value)}
                          placeholder="e.g. Lead Form Submit, Checkout Success, WhatsApp Click"
                          className="w-full px-3 py-2 text-xs rounded-xl border border-slate-200 focus:outline-none focus:border-blue-500"
                        />
                      </div>
                      ...
                      <div className="space-y-1">
                        ...
                        <button
                          ...
                          className={`w-full flex items-center justify-center gap-1.5 px-3 py-2 rounded-xl text-xs font-bold transition-all border cursor-pointer ${
                            goalIsActive
                              ? "bg-emerald-50 text-emerald-700 border-emerald-200"
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                      <div className="space-y-1">
                        <label className="text-xs font-bold text-slate-700">Conversion Type</label>
                        <select
                          value={goalType}
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                      <div className="space-y-1">
                        <label className="text-xs font-bold text-slate-700">Conversion Value</label>
                        <input
                          type="text"
                          value={goalValue}
                      ...
                      <div className="space-y-1">
                        ...
                        <input
                          ...
                          className="w-full px-3 py-2 text-xs rounded-xl border border-slate-200 focus:outline-none focus:border-blue-500 font-mono uppercase"
                        />
                      </div>

                      <div className="space-y-1">
                        <label className="text-xs font-bold text-slate-700">Optimization Goal</label>
            ...
            {activeTab === "brand_profile" && (
              <div className="animate-fadeIn space-y-6 transition-all duration-300">
                ...
                <div className="p-6 rounded-3xl bg-white border border-slate-200 shadow-sm space-y-5">
                  ...
                  {/* Brand Description */}
                  <div className="space-y-1">
                    <label className="text-xs font-bold text-slate-700">Brand Description</label>
                    <textarea
                      rows={3}
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {/* Brand Website */}
                    <div className="space-y-1">
                      <label className="text-xs font-bold text-slate-700">Brand Website</label>
                      <input
                        type="url"
                        value={brandWebsite}
                    ...
                    <div className="space-y-1">
                      ...
                      <div className="flex items-center gap-2">
                        <input
                          ...
                          className="flex-1 px-3 py-2 text-xs rounded-xl border border-slate-200 focus:outline-none focus:border-blue-500 font-mono text-[11px]"
                        />
                        {logoUrl.trim() && (
                  ...
                  <div className="space-y-2">
                    <label className="text-xs font-bold text-slate-700">Brand Colors</label>
                    <div className="flex items-center gap-2">
                      <input
                        type="text"
                        ...
                        className="flex-1 px-3 py-2 text-xs rounded-xl border border-slate-200 focus:outline-none focus:border-blue-500 font-mono"
                      />
                      <button
                        type="button"
                    ...
                    {brandColors.length > 0 && (
                      <div className="flex items-center gap-2 flex-wrap pt-1">
                        {brandColors.map((color, idx) => (
                          <span
                            key={idx}
                            className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-xl bg-slate-100 text-slate-800 text-xs font-semibold border border-slate-200"
                          >
                            <span
                  ...
                  {/* Brand Keywords */}
                  <div className="space-y-2">
                    <label className="text-xs font-bold text-slate-700">Brand Keywords</label>
                    <div className="flex items-center gap-2">
                      <input
                        type="text"
                        ...
                        placeholder="e.g. luxury skincare, enterprise CRM, organic coffee"
                    ...
                    {brandKeywords.length > 0 && (
                      <div className="flex items-center gap-1.5 flex-wrap pt-1">
                        {brandKeywords.map((keyword, idx) => (
                          <span
                            key={idx}
                            className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-lg bg-blue-50 text-blue-700 text-xs font-semibold border border-blue-100"
                          >
                            <span>{keyword}</span>
                            <button
                              type="button"
                ...
                <div className="p-6 rounded-3xl bg-white border border-slate-200 shadow-sm space-y-4">
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      <h2 className="text-sm font-bold text-slate-900">Brand Voice &amp; Tone</h2>
                      <p className="text-xs text-slate-500">
                        Select all tone and personality attributes that best represent how the brand communicates in Google Ads headlines, descriptions, and sitelinks.
                      </p>
                    </div>
                  </div>
                ...
                {/* 3. Messaging Guidelines & Advertising Rules Card */}
                <div className="p-6 rounded-3xl bg-white border border-slate-200 shadow-sm space-y-5">
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      <h2 className="text-sm font-bold text-slate-900">Messaging Guidelines &amp; Advertising Rules</h2>
                      <p className="text-xs text-slate-500">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {/* Preferred CTA */}
                    <div className="space-y-1">
                      <label className="text-xs font-bold text-slate-700">Preferred CTA</label>
                      <input
                        type="text"
                        value={preferredCta}
                        onChange={(e) => setPreferredCta(e.target.value)}
                        placeholder="e.g. Book Free Consultation, Get Started Today, Shop Now"
                        className="w-full px-3 py-2 text-xs rounded-xl border border-slate-200 focus:outline-none focus:border-blue-500"
                      />
                    </div>
                    ...
                    {/* Promotional Style */}
                    <div className="space-y-1">
                      <label className="text-xs font-bold text-slate-700">Promotional Style</label>
                      <input
                        type="text"
                        value={promotionalStyle}
                        onChange={(e) => setPromotionalStyle(e.target.value)}
                        placeholder="e.g. Value-driven / Educational / Urgent / Direct Response"
                        className="w-full px-3 py-2 text-xs rounded-xl border border-slate-200 focus:outline-none focus:border-blue-500"
                      />
                    </div>
                  </div>
                  ...
                  {/* Preferred Messaging */}
                  <div className="space-y-1">
                    <label className="text-xs font-bold text-slate-700">Preferred Messaging</label>
                    <textarea
                      rows={2}
                  ...
                  {/* Key Brand USPs */}
                  <div className="space-y-2">
                    <label className="text-xs font-bold text-slate-700">Key Brand USPs</label>
                    <div className="flex items-center gap-2">
                      <input
                        type="text"
                        value={brandUspInput}
                        ...
                        placeholder="e.g. 24/7 Support, 100% Organic, Free Nationwide Shipping"
                        className="flex-1 px-3 py-2 text-xs rounded-xl border border-slate-200 focus:outline-none focus:border-blue-500"
                      />
                      <button
                        type="button"
                    ...
                    {brandUsps.length > 0 && (
                      <div className="flex items-center gap-1.5 flex-wrap pt-1">
                        {brandUsps.map((usp, idx) => (
                          <span
                            key={idx}
                            className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-lg bg-emerald-50 text-emerald-700 text-xs font-semibold border border-emerald-100"
                          >
                            <span>{usp}</span>
                            <button
                              type="button"
                              onClick={() => handleRemoveBrandUsp(usp)}
                              className="text-emerald-400 hover:text-rose-600 ml-1 cursor-pointer"
                            >
                              <X className="w-3 h-3" />
                            </button>
                          </span>
                  ...
                  {/* Words / Phrases to Prefer & Words / Phrases to Avoid */}
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {/* Prefer */}
                    <div className="space-y-2">
                      <label className="text-xs font-bold text-slate-700">Words / Phrases to Prefer</label>
                      <div className="flex items-center gap-2">
                        <input
                          type="text"
                          ...
                          placeholder="e.g. Certified, Guaranteed, Authentic"
                          className="flex-1 px-3 py-2 text-xs rounded-xl border border-slate-200 focus:outline-none focus:border-blue-500"
                        />
                        <button
                          type="button"
                    ...
                    {/* Avoid */}
                    <div className="space-y-2">
                      <label className="text-xs font-bold text-slate-700">Words / Phrases to Avoid</label>
                      <div className="flex items-center gap-2">
                        <input
                          type="text"
                          ...
                          placeholder="e.g. Cheap, Lowest price, Risk-free"
                          className="flex-1 px-3 py-2 text-xs rounded-xl border border-slate-200 focus:outline-none focus:border-blue-500"
                        />
                        <button
                          type="button"
                      ...
                      {wordsToAvoid.length > 0 && (
                        <div className="flex items-center gap-1.5 flex-wrap pt-1">
                          {wordsToAvoid.map((w, idx) => (
                            <span
                              key={idx}
                              className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-lg bg-rose-50 text-rose-700 text-xs font-semibold border border-rose-100"
                            >
                              <span>{w}</span>
                              <button
                                type="button"
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {/* Do's */}
                    <div className="space-y-2">
                      <label className="text-xs font-bold text-slate-700">Advertising Do&apos;s</label>
                      <div className="flex items-center gap-2">
                        <input
                          type="text"
                          value={adDoInput}
                          ...
                          placeholder="e.g. Highlight ISO certification, Mention warranty"
                          className="flex-1 px-3 py-2 text-xs rounded-xl border border-slate-200 focus:outline-none focus:border-blue-500"
                        />
                        <button
                          type="button"
                    ...
                    {/* Don'ts */}
                    <div className="space-y-2">
                      <label className="text-xs font-bold text-slate-700">Advertising Don&apos;ts</label>
                      <div className="flex items-center gap-2">
                        <input
                          type="text"
                          value={adDontInput}
                      ...
                      {advertisingDonts.length > 0 && (
                        <div className="flex items-center gap-1.5 flex-wrap pt-1">
                          {advertisingDonts.map((item, idx) => (
                            <span
                              key={idx}
                              className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-lg bg-amber-50 text-amber-800 text-xs font-semibold border border-amber-200"
                            >
                              <span>{item}</span>
                              <button
                                type="button"
                                onClick={() => handleRemoveAdDont(item)}
                                className="text-amber-500 hover:text-rose-700 ml-1 cursor-pointer"
                              >
                                <X className="w-3 h-3" />
                              </button>
                            </span>
                  ...
                  {/* Rules: Discount / Offer Rules & Price Rules */}
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div className="space-y-1">
                      <label className="text-xs font-bold text-slate-700">Discount / Offer Messaging Rules</label>
                      <textarea
                        rows={2}
                        value={discountRules}
                        onChange={(e) => setDiscountRules(e.target.value)}
                        placeholder="e.g. Never advertise discounts > 25%; always include 'T&C Apply'..."
                        className="w-full px-3 py-2 text-xs rounded-xl border border-slate-200 focus:outline-none focus:border-blue-500 leading-relaxed"
                      />
                    </div>

                    <div className="space-y-1">
                      <label className="text-xs font-bold text-slate-700">Price Messaging Rules</label>
                      <textarea
                        rows={2}
                        value={priceRules}
                        onChange={(e) => setPriceRules(e.target.value)}
                        placeholder="e.g. Always display 'Starting at ₹499'; avoid specific packages without disclaimers..."
                        className="w-full px-3 py-2 text-xs rounded-xl border border-slate-200 focus:outline-none focus:border-blue-500 leading-relaxed"
                      />
                    </div>
                  </div>
                  ...
                  <div className="space-y-1">
                    <label className="text-xs font-bold text-slate-700">Additional Brand Notes</label>
                    <textarea
                      rows={2}
                      value={brandAdditionalNotes}
                      onChange={(e) => setBrandAdditionalNotes(e.target.value)}
                      placeholder="Any additional instructions, target personality details, or compliance requirements..."
                      className="w-full px-3 py-2 text-xs rounded-xl border border-slate-200 focus:outline-none focus:border-blue-500 leading-relaxed"
                    />
                  </div>
                </div>
            ...
            {/* TAB CONTENT: COMPETITOR INTELLIGENCE */}
            ...
            {/* TAB CONTENT: SEO & KEYWORD INTELLIGENCE */}
            ...
            {/* TAB CONTENT: BUSINESS FAQS & KEY INFORMATION */}
            {activeTab === "faqs" && (
              <div className="animate-fadeIn space-y-6 transition-all duration-300">
                <div className="p-6 rounded-3xl bg-white border border-slate-200 shadow-sm space-y-6">
                  {/* Header */}
            ...
            {/* TAB CONTENT: AI SUGGESTIONS & INTELLIGENCE MAPPING */}
            ...
            {/* TAB CONTENT: WEBSITES & SUB-PAGES */}
            {activeTab === "websites" && (
              <div className="animate-fadeIn space-y-6 transition-all duration-300">
                <div className="p-6 rounded-3xl bg-white border border-slate-200 shadow-sm space-y-6">
                <div className="flex items-center justify-between flex-wrap gap-2">
                  <div>
                    <h2 className="text-sm font-bold text-slate-900">Websites &amp; Discovered Sub-Pages</h2>
                    <p className="text-xs text-slate-500">
                      Primary Website + up to 14 additional websites. Groq AI extracts up to 15 sub-pages per site.
                    </p>
                  </div>
                  <span
                  ...
                  >
                    {totalWebsitesCount} / 15 Total Websites
                  </span>
                </div>
                ...
                <div className="p-5 rounded-2xl bg-blue-50/40 border border-blue-200 space-y-3">
                  ...
                  <div className="relative flex items-center">
                    <input
                      ...
                      className="w-full px-4 py-2.5 text-xs rounded-xl border border-slate-200 bg-white font-mono text-slate-800 focus:outline-none focus:border-blue-500 pr-9"
                    />
                    {primaryWebsite && (
                      <button
                        type="button"
                        ...
                        className="absolute right-2.5 text-slate-400 hover:text-rose-600 cursor-pointer p-1 transition-colors"
                        title="Clear website & sub-pages"
                      >
                        <X className="w-3.5 h-3.5" />
                      </button>
                  ...
                  {primarySubPages.length > 0 && (
                    <div className="pt-2 border-t border-blue-100">
                      <button
                        ...
                        className="text-xs font-bold text-blue-800 flex items-center justify-between w-full py-1 cursor-pointer"
                      >
                        <span>Discovered Sub-Pages ({primarySubPages.length} found)</span>
                        {expandedWebsiteUrl === primaryWebsite ? (
                      ...
                      {expandedWebsiteUrl === primaryWebsite && (
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 mt-2 pt-2 border-t border-blue-100/60 max-h-56 overflow-y-auto">
                          {primarySubPages.map((sp, idx) => (
                            <div
                              key={idx}
                              className="p-2.5 rounded-xl bg-white border border-blue-100 text-xs flex flex-col justify-between"
                            >
                ...
                <div className="space-y-3">
                  ...
                  {additionalWebsites.length === 0 ? (
                    <div className="p-8 rounded-2xl border border-dashed border-slate-200 text-center text-xs text-slate-400">
                      No additional websites configured. Add regional landing pages, product microsites, or brand stores below.
                    </div>
                  ) : (
                          ...
                          <div className="flex items-center justify-between flex-wrap gap-2">
                            ...
                            <div className="flex items-center gap-1.5">
                              <button
                                ...
                                className="inline-flex items-center gap-1 px-3 py-1 text-xs font-bold rounded-xl bg-blue-50 text-blue-700 hover:bg-blue-100 transition-all cursor-pointer"
                              >
                                {isAnalyzing ? (
                                  <Loader2 className="w-3.5 h-3.5 animate-spin" />
                                ) : (
                          ...
                          {isExpanded && (
                            <div className="pt-2 border-t border-slate-100">
                              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 max-h-56 overflow-y-auto">
                                {w.subPages.length === 0 ? (
                                  <p className="text-[11px] text-slate-400 italic">No sub-pages discovered yet. Click Analyze.</p>
                                ) : (
                                  w.subPages.map((sp, sIdx) => (
                                    <div
                                      key={sIdx}
                                      className="p-2.5 rounded-xl bg-slate-50 border border-slate-200 text-xs flex flex-col justify-between"
                                    >
                                      <span className="font-semibold text-slate-800 truncate">{sp.text}</span>
                                      <a
                                        href={sp.url}
                  ...
                  {totalWebsitesCount < 15 ? (
                    <div className="flex gap-2 pt-2">
                      <input
                        ...
                        placeholder="https://store.example.com or https://blog.example.com"
                        className="flex-1 px-3.5 py-2.5 text-xs rounded-xl border border-slate-200 bg-white font-mono text-slate-800 focus:outline-none focus:border-blue-500"
                      />
                      <button
                        type="button"
                    ...
                    <p className="text-xs text-amber-700 font-semibold p-3.5 rounded-xl bg-amber-50 border border-amber-200">
                      Maximum website quota reached (15 websites). Remove an additional website to add another.
                    </p>
                  )}
                </div>
              </div>
                ...
                >
                  ...
                  {hasAppAccount && (
                    <div className="space-y-4 pt-4 border-t border-blue-200/60 animate-fadeIn">
                      <label className="text-xs font-bold text-slate-800 block">
                        Connected Applications ({appDetails.length})
                      </label>
                        ...
                        appDetails.map((app, aIdx) => (
                          ...
                          >
                            <div className="flex items-center gap-3 min-w-0">
                              ...
                              >
                                {app.platform === "IOS" ? "iOS App" : "Android"}
                              </span>
                              <div className="min-w-0">
            ...
            {activeTab === "overview" && profile && (
              <div className="animate-fadeIn space-y-6 transition-all duration-300">
                <div className="p-6 rounded-3xl bg-white border border-slate-200 shadow-sm space-y-4">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-xs">
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <span className="block text-[11px] text-slate-400">Team Member</span>
                ...
                <div className="p-5 rounded-3xl bg-blue-50/60 border border-blue-200 flex items-start gap-3">
                  <ShieldCheck className="w-5 h-5 text-blue-600 shrink-0 mt-0.5" />
                  <div className="text-xs text-blue-900 space-y-1">
                    <p className="font-bold">Customer Isolation Guarantee</p>
...
export default function ProfilePage() {
  return (
    <Suspense
      fallback={
        <div className="flex-1 h-full min-h-0 bg-slate-50 flex items-center justify-center text-xs font-semibold text-slate-500">
          Loading Business &amp; Marketing Profile...
        </div>
      }
    >
      <ProfilePageContent />
