...
import { GoogleAdsKeywordPlannerSection } from "./GoogleAdsKeywordPlannerSection";
import { GoogleAdsPerformancePlannerSection } from "./GoogleAdsPerformancePlannerSection";
...
export function GoogleAdsReportingSection({ customerId, orgId }: GoogleAdsReportingSectionProps) {
  const [activeReport, setActiveReport] = useState<"auction-insights" | "landing-pages" | "custom-reports" | "keyword-planner" | "performance-planner">("auction-insights");
  ...
  }, [activeReport, fetchAuctionInsights]);
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white p-4 rounded-2xl border border-slate-200 shadow-2xs">
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <button
            onClick={() => setActiveReport("custom-reports")}
            className={`flex items-center gap-2 px-4 py-2 rounded-xl text-xs font-bold transition-all cursor-pointer ${
              activeReport === "custom-reports"
                ? "bg-blue-600 text-white shadow-xs"
                : "bg-slate-100 text-slate-700 hover:bg-slate-200"
            }`}
          >
          ...
          <button
            onClick={() => setActiveReport("keyword-planner")}
            className={`flex items-center gap-2 px-4 py-2 rounded-xl text-xs font-bold transition-all cursor-pointer ${
              activeReport === "keyword-planner"
                ? "bg-blue-600 text-white shadow-xs"
                : "bg-slate-100 text-slate-700 hover:bg-slate-200"
            }`}
          >
          ...
          <button
            onClick={() => setActiveReport("performance-planner")}
            className={`flex items-center gap-2 px-4 py-2 rounded-xl text-xs font-bold transition-all cursor-pointer ${
              activeReport === "performance-planner"
                ? "bg-blue-600 text-white shadow-xs"
                : "bg-slate-100 text-slate-700 hover:bg-slate-200"
            }`}
          >
      ...
      {activeReport === "auction-insights" && (
        <div className="bg-white p-4 rounded-2xl border border-slate-200 shadow-2xs space-y-3">
          ...
          >
            <RefreshCw
              className={`h-4 w-4 ${
                auctionLoading ? "animate-spin text-blue-600" : ""
              }`}
            />
          </button>
        </div>
      ...
      {/* ═══════════════════════════════════════════════════════════════════ */}
      {/* 2. LANDING PAGE PERFORMANCE (Dedicated API v24 Component) */}
      {/* ═══════════════════════════════════════════════════════════════════ */}
      {activeReport === "landing-pages" && (
        <GoogleAdsLandingPagesSection
      ...
      {/* ═══════════════════════════════════════════════════════════════════ */}
      {/* 3. CUSTOM REPORT BUILDER (Dedicated API v24 Component) */}
      {/* ═══════════════════════════════════════════════════════════════════ */}
      {activeReport === "custom-reports" && (
        <GoogleAdsCustomReportsSection
          customerId={customerId}
          orgId={orgId}
          campaigns={campaignsList}
        />
      ...
      {/* ═══════════════════════════════════════════════════════════════════ */}
      {/* 4. KEYWORD PLANNER / KEYWORD IDEAS (Dedicated API v24 Component) */}
      {/* ═══════════════════════════════════════════════════════════════════ */}
      {activeReport === "keyword-planner" && (
        <GoogleAdsKeywordPlannerSection
          customerId={customerId}
          orgId={orgId}
        />
      )}
      ...
      {/* ═══════════════════════════════════════════════════════════════════ */}
      {/* 5. PERFORMANCE PLANNER / CAMPAIGN FORECAST (Dedicated API v24 Component) */}
      {/* ═══════════════════════════════════════════════════════════════════ */}
      {activeReport === "performance-planner" && (
        <GoogleAdsPerformancePlannerSection
          customerId={customerId}
          orgId={orgId}
        />
      )}
