...
// 1. Consultative Advisor code
const consultativeBlock = `    // 2.0C CONSULTATIVE ADVISOR INTENTS (Senior Media Buyer guidance for Goal, Destination & Budget)
    ...
    if (isGoalConsultation && (!state.draft.campaign.promotedService || !state.draft.campaign.name || /AI Meta Campaign|Meta Ad Campaign Blueprint/i.test(state.draft.campaign.name))) {
...
code = code.replace(
  '    // 2.1 Priority Phone Number Extraction Regex',
  `${consultativeBlock}\n    // 2.1 Priority Phone Number Extraction Regex`
);

// 2. Update phone response locPrompt with style checks (Hinglish/Marathlish Latin script)
const oldLocPromptCode = `          if (detectedLang.code === "mr") {
...
const newLocPromptCode = `          if (style.isMarathlish) {
            locPrompt = \`WhatsApp number (\${formattedNum}) save zala ahe. 📱\\n\\nYa campaign sathi konatya city kinva area madhe ad dakhavaychi ahe?\`;
            locOptions = [
              { label: "📍 All India (Poora Bharat)", value: "ALL_INDIA" },
              { label: "📍 Mumbai & Pune", value: "Mumbai, Pune" },
              { label: "📍 Nagpur", value: "Nagpur" },
              { label: "🌐 Bulk Locations (Countries, Cities, PIN & Radius)", value: "OPEN_BULK_LOCATIONS" },
            ];
          } else if (style.isHinglish) {
            locPrompt = \`WhatsApp number (\${formattedNum}) save ho gaya hai. 📱\\n\\nIs campaign ke liye kis city ya area ko target karna chahte hain?\`;
            locOptions = [
              { label: "📍 All India (Poora Bharat)", value: "ALL_INDIA" },
              { label: "📍 Mumbai & Pune", value: "Mumbai, Pune" },
...
code = code.replace(oldLocPromptCode, newLocPromptCode);

fs.writeFileSync('src/services/meta-ads/metaAIConversationService.ts', code, 'utf8');
console.log('Successfully added consultative intents and phone script fixes!');
