...
async function runTrafficFullDiagnostic() {
  console.log("==================================================");
  console.log("🚀 COMPREHENSIVE TRAFFIC CAMPAIGN DIAGNOSTIC");
  console.log("==================================================");

  const testResults = [];

  // STEP 1: Check Traffic Campaign Schema Dropdown Options
  try {
  ...
  try {
    console.log("\n2️⃣ Testing POST /api/meta-ads/campaigns (Traffic Objective)...");
    const createRes = await axios.post(`${BASE}/campaigns`, {
      organizationId: ORG_ID,
      name: `Full Diagnostic Traffic Campaign - ${Date.now()}`,
      ...
      creativeHeadline: "Drive High Quality Website Traffic",
      creativeBody: "Click to explore our latest products and features on our official website.",
  ...
  try {
    ...
    if (found) {
      console.log(`✅ Passed: Created Traffic campaign present in total ${list.length} listed campaigns.`);
  ...
  try {
    ...
    console.log(`✅ Passed: Selected Objective=${sv.objective}, Budget=₹${sv.dailyBudget}, Goal=${sv.optimizationGoal}, Destination=${sv.destinationType}.`);
  ...
  console.log("\n==================================================");
  console.log("📊 TRAFFIC CAMPAIGN ALL ENDPOINTS & DROPDOWNS RESULT:");
  console.log("==================================================");
  console.table(testResults);
}

runTrafficFullDiagnostic();
