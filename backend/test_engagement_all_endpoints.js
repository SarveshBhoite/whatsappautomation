...
async function runEngagementFullDiagnostic() {
  console.log("==================================================");
  console.log("💬 COMPREHENSIVE ENGAGEMENT CAMPAIGN DIAGNOSTIC");
  console.log("==================================================");

  const testResults = [];

  // STEP 1: Check Engagement Campaign Schema Dropdown Options
  try {
    ...
    console.log(`✅ Dropdown Audit: Messaging Destinations (${destinationOpt.length}), Optimization Goals (${optimizationOpt.length}), Bid Strategies (${bidOpt.length}), CTAs (${ctaOpt.length}), Categories (${categoryOpt.length}).`);
  ...
  // STEP 2: Create Engagement Campaign
  let campaignId = null;
  let metaCampaignId = null;
  try {
    console.log("\n2️⃣ Testing POST /api/meta-ads/campaigns (Engagement Objective)...");
    const createRes = await axios.post(`${BASE}/campaigns`, {
      organizationId: ORG_ID,
      name: `Full Diagnostic Engagement Campaign - ${Date.now()}`,
      ...
      creativeHeadline: "Connect with Customer Service Team",
      creativeBody: "Start an instant conversation on WhatsApp for instant automated assistance.",
  ...
  try {
    ...
    if (found) {
      console.log(`✅ Passed: Created Engagement campaign present in total ${list.length} listed campaigns.`);
  ...
  try {
    ...
    console.log(`✅ Passed: Selected Objective=${sv.objective}, Budget=₹${sv.dailyBudget}, Goal=${sv.optimizationGoal}, Destination=${sv.destinationType}, CTA=${sv.callToAction}.`);
  ...
  console.log("\n==================================================");
  console.log("📊 ENGAGEMENT CAMPAIGN ALL ENDPOINTS & DROPDOWNS RESULT:");
  console.log("==================================================");
  console.table(testResults);
}

runEngagementFullDiagnostic();
