...
router.post("/instagram/embedded-signup/callback", async (req: Request, res: Response) => {
  try {
    ...
    let accessToken = "";
    // If redirectUri is explicitly passed as empty string (""), the code was generated by the client FB JS SDK (FB.login) without a redirect_uri.
    // If redirectUri has a value, it came from the OAuth dialog popup flow.
    const isSdkCode = redirectUri === "";
    const cleanRedirectUri = isSdkCode ? undefined : (redirectUri || "https://crm.jisnudigital.com/settings?tab=instagram");
    ...
    try {
      ...
      if (cleanRedirectUri) {
        exchangeParams.redirect_uri = cleanRedirectUri;
      }

      console.log(`[IG EMBEDDED SIGNUP] Exchanging code (isSdkCode: ${isSdkCode}, redirect_uri: ${cleanRedirectUri || 'NONE'})...`);
      const tokenResponse = await axios.get("https://graph.facebook.com/v21.0/oauth/access_token", {
        params: exchangeParams,
      });
      accessToken = tokenResponse.data.access_token;
    } catch (err1: any) {
      console.warn("[IG EMBEDDED SIGNUP] Token exchange initial attempt failed:", err1?.response?.data?.error?.message || err1.message);
      // Secondary attempt with reversed redirect_uri parameter
      try {
        ...
        if (!cleanRedirectUri) {
          fallbackParams.redirect_uri = "https://crm.jisnudigital.com/settings?tab=instagram";
        }
        const tokenResponse = await axios.get("https://graph.facebook.com/v21.0/oauth/access_token", {
        ...
        console.warn("[IG EMBEDDED SIGNUP] Token exchange secondary attempt failed:", err2?.response?.data?.error?.message || err2.message);
      }
    }
