...
router.post("/comment-automations/:id/test", async (req: Request, res: Response) => {
  try {
    ...
    const testUsername = typeof req.body.testUsername === "string" ? req.body.testUsername : "";
    const testCommentText = typeof req.body.testCommentText === "string" ? req.body.testCommentText : "";

    const simulation = await InstagramCommentEngine.evaluateTestSimulation(String(id), testUsername, testCommentText);
