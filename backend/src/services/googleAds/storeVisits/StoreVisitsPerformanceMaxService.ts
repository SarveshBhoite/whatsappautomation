  ...
  public static async createCampaign(organizationId: string, customerId: string, payload: any) {
    ...
    if (isAiGuided) {
      if (!finalUrl || !String(finalUrl).trim()) {
        throw new Error("Final URL is required for AI Guided Store Visits Performance Max.");
      }
    ...
    if (!isAiGuided && (safeFinalUrl.includes("example.com") || safeFinalUrl.includes("localhost") || safeFinalUrl.includes("example.org"))) {
    ...
    const safeHeadlines = isAiGuided
      ...
      : (cleanedHeadlines.length >= 3 ? cleanedHeadlines : [...cleanedHeadlines, "Visit Our Store", "Exclusive In-Store Offers", "Find Nearby Stores"]).slice(0, 5);

    const safeLongHeadlines = isAiGuided
      ? cleanedLongHeadlines.slice(0, 5)
      : (cleanedLongHeadlines.length >= 1 ? cleanedLongHeadlines : ["Experience our in-store collection and personalized service today."]).slice(0, 5);

    const safeDescriptions = isAiGuided
      ? cleanedDescriptions.slice(0, 5)
      : (cleanedDescriptions.length >= 2 ? cleanedDescriptions : [...cleanedDescriptions, "Visit our store today for special promotions.", "Get directions and visit us for exclusive discounts."]).slice(0, 5);

    const safeBusinessName = isAiGuided
      ? GoogleAdsBaseService.cleanAdText(String(businessName), 25)
      : (GoogleAdsBaseService.cleanAdText(businessName || "My Store", 25) || "My Store");
    ...
    try {
      ...
      // In AI Guided, strict explicit asset types are required - no cross-asset borrowing or default placeholders!
      if (!isAiGuided) {
      ...
      for (const loc of locList) {
        ...
        const constantId = StoreVisitsPerformanceMaxService.GEO_TARGET_CONSTANT_MAP[normLoc] || (/^\d+$/.test(String(loc)) ? String(loc) : null);
        if (constantId) {
      ...
      for (const lang of langList) {
        ...
        const constantId = StoreVisitsPerformanceMaxService.LANGUAGE_CONSTANT_MAP[normLang] || (/^\d+$/.test(String(lang)) ? String(lang) : null);
        if (constantId) {
          criteriaOperations.push({
            create: {
              campaign: campaignRef,
      ...
      if (criteriaOperations.length > 0) {
        try {
          ...
          console.warn("[Store Visits PMax] Warning: Failed to mutate campaign criteria:", critErr?.response?.data || critErr.message);
        }
      }
