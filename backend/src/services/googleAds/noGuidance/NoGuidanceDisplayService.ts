...
export class NoGuidanceDisplayService extends GoogleAdsBaseService {
  ...
  public static async resolveGeoTargetConstant(
    ...
    try {
      ...
      console.warn(`[NoGuidanceDisplayService] geoTargetConstants:suggest failed for "${trimmed}":`, e?.message || e);
    }
  ...
  public static async createCampaign(organizationId: string, customerId: string, payload: any) {
    ...
    const apiResult: any = { campaignId: `noguidance-display-${Date.now()}` };
    ...
    try {
      ...
      if (!budgetRef) {
        throw new Error("Failed to create CampaignBudget for No Guidance Display.");
      }
      ...
      if (!campaignRef) {
        throw new Error("Failed to create No Guidance Display Campaign.");
      }
      ...
      for (const loc of rawLocs) {
        ...
        const geoId = await NoGuidanceDisplayService.resolveGeoTargetConstant(locStr, headers);
        if (geoId) {
      ...
      for (const lang of rawLangs) {
        const langId = NoGuidanceDisplayService.resolveLanguageConstant(String(lang));
        if (langId) {
          campaignCriterionOps.push({
            create: {
              campaign: campaignRef,
      ...
      if (Array.isArray(effectiveAdSchedule) && effectiveAdSchedule.length > 0) {
        const scheduleCriteria = NoGuidanceDisplayService.buildAdScheduleCriteria(effectiveAdSchedule);
      ...
      for (const source of allContentExclusionSources) {
        for (const [label, isExcluded] of Object.entries(source)) {
          if (isExcluded) {
            const mappedLabelEnum = NoGuidanceDisplayService.CONTENT_LABEL_MAP[label];
      ...
      if (campaignCriterionOps.length > 0) {
        try {
          ...
          console.warn("[NoGuidanceDisplayService] campaignCriteria mutate warning:", campCritErr?.response?.data || campCritErr.message);
        }
      }
      ...
      if (!adGroupRef) {
        throw new Error("Failed to create AdGroup for No Guidance Display.");
      }
      ...
      if (demographicsIncome && typeof demographicsIncome === "object") {
        for (const [tier, isIncluded] of Object.entries(demographicsIncome)) {
          if (isIncluded === false) {
            const mappedIncome = NoGuidanceDisplayService.INCOME_RANGE_MAP[tier];
            if (mappedIncome) {
              adGroupCriterionOps.push({
                create: {
                  adGroup: adGroupRef,
      ...
      if (adGroupCriterionOps.length > 0) {
        try {
          ...
          console.warn("[NoGuidanceDisplayService] adGroupCriteria mutate warning:", critErr?.response?.data || critErr.message);
        }
      }
      ...
      for (const vid of rawVideos) {
        ...
        if (youtubeVideoId) {
          try {
            ...
            console.warn("[NoGuidanceDisplayService] YouTube video asset creation warning:", vidErr?.response?.data || vidErr.message);
          }
        }
      }
      ...
      console.log("[DEBUG] RDA Payload being sent to Google Ads API:", JSON.stringify(adGroupAdPayload, null, 2));
      ...
      console.error("[Google Ads API Error for No Guidance Display]:", GoogleAdsBaseService.formatGoogleAdsError(apiErr));
      if (apiErr?.response?.data) {
        console.error("[RAW GOOGLE ADS API ERROR]:", JSON.stringify(apiErr.response.data, null, 2));
      }
    ...
    const localCampaign = await this.saveCampaignToDatabase({
      organizationId,
      customerId,
      googleAdsCampaignId: apiResult.campaignId || `noguidance-display-${Date.now()}`,
      name: campaignName,
    ...
    return {
      message: "No Guidance Display Campaign created successfully (Paused)",
      campaign: {
        ...localCampaign,
