...
export class NoGuidanceDisplayService extends GoogleAdsBaseService {
  public static async createCampaign(organizationId: string, customerId: string, payload: any) {
    const {
      campaignName = "No Guidance Display",
    ...
    if (validHeadlines.length < 1) {
      validHeadlines.push("Quality Services & Products");
    }

    const validDescriptions = descriptions.filter((d: any) => d && typeof d === "string" && d.trim().length > 0);
    if (validDescriptions.length < 1) {
      validDescriptions.push("Discover top offers and tailored solutions for all your requirements.");
    }
    ...
    let apiResult: any = { campaignId: `noguidance-display-${Date.now()}` };
    ...
    try {
      ...
      for (const img of rawImages) {
        ...
        if (fieldType === "MARKETING_IMAGE") {
          ...
          if (typeof raw === "string" && raw.includes("ik.imagekit.io")) {
            ...
            const ref = await this.uploadImageAsset(organizationId, customerId, `NoG_Disp_Sq_${Date.now()}`, raw);
      ...
      const safeHeadlines = (cleanedHeadlines.length > 0 ? cleanedHeadlines : ["Quality Products Online"])
        .slice(0, 5)
      ...
      console.error("[Google Ads API Error for No Guidance Display]:", formatted);
