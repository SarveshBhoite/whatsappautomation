    ...
    try {
      ...
      try {
        ...
        if (err?.response?.data) {
          console.error("[Google Ads API Error for No Guidance Search Ad Group]:", JSON.stringify(err.response.data, null, 2));
        } else {
          console.error("[No Guidance Search Ad Group API Error]:", err.message);
        }
        throw err;
      }
    } catch (apiErr: any) {
