...
export class NoGuidanceSearchService extends GoogleAdsBaseService {
  ...
  public static async resolveGeoTargetConstant(
    ...
    try {
      ...
      console.warn(`[NoGuidanceSearchService] geoTargetConstants:suggest lookup failed for "${trimmed}":`, e?.message || e);
    }
  ...
  public static async createCampaign(organizationId: string, customerId: string, payload: any) {
    ...
    const {
      campaignName = "No Guidance Search",
    ...
    let apiResult: any = { campaignId: `noguidance-search-${Date.now()}` };
    ...
    try {
      ...
      for (const lang of langList) {
        ...
        const langConstantId = NoGuidanceSearchService.LANGUAGE_CONSTANT_MAP[normLang] || (/^\d+$/.test(String(lang)) ? String(lang) : null);
        if (langConstantId) {
          campaignCriteriaOperations.push({
            create: {
              campaign: campaignRef,
      ...
      if (scheduleList.length > 0) {
        const scheduleCriteria = NoGuidanceSearchService.buildAdScheduleCriteria(scheduleList);
      ...
      if (apiErr?.response?.data) {
        console.error(
          "[Google Ads API Error for No Guidance Search]:",
          JSON.stringify(apiErr.response.data, null, 2)
        );
      } else {
        console.error("[No Guidance Search API Error]:", apiErr.message);
      }
      throw apiErr;
    }
    ...
    return {
      message: "No Guidance Search Campaign created successfully (Paused)",
      campaign: {
        ...localCampaign,
