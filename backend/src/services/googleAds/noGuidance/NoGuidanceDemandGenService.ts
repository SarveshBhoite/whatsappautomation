...
export class NoGuidanceDemandGenService extends GoogleAdsBaseService {
  ...
  public static async resolveGeoTargetConstant(
    ...
    try {
      ...
      console.warn(`[NoGuidanceDemandGenService] geoTargetConstants:suggest failed for "${trimmed}":`, e?.message || e);
    }
  ...
  public static async createCampaign(organizationId: string, customerId: string, payload: any) {
    ...
    // ── 1. STRICT PRE-FLIGHT VALIDATION (STRICT MODE: NO FAKES / SILENT FALLBACKS) ──
    ...
    try {
      ...
      if (Array.isArray(adSchedule) && adSchedule.length > 0) {
        const scheduleCriteria = NoGuidanceDemandGenService.buildAdScheduleCriteria(adSchedule);
      ...
      if (campaignCriterionOps.length > 0) {
        try {
          ...
          console.warn("[NoGuidanceDemandGenService] campaignCriteria mutate warning:", campCritErr?.response?.data || campCritErr.message);
          if (isAiGuided) {
            throw campCritErr;
          }
        }
      ...
      if (audience && typeof audience === "object") {
        const dimensions = NoGuidanceDemandGenService.buildAudienceDimensions(audience, cid);
        if (dimensions.length > 0) {
          ...
          try {
              ...
              operations: [
                {
                  create: {
                    name: audienceName,
                    description: `Audience for No Guidance Demand Gen Campaign ${effectiveCampaignName}`.slice(0, 255),
                    dimensions
                  }
                }
              ]
            ...
            console.warn("[NoGuidanceDemandGenService] Audience resource creation warning:", audErr?.response?.data || audErr.message);
            if (isAiGuided) {
              throw audErr;
            }
          }
      ...
      if (adGroupCriterionOps.length > 0) {
        try {
          ...
          console.warn("[NoGuidanceDemandGenService] adGroupCriteria mutate warning:", critErr?.response?.data || critErr.message);
          if (isAiGuided) {
            throw critErr;
          }
        }
      ...
      if (hasExtensions) {
          ...
          for (const sl of sitelinks) {
            ...
            if (linkText && slUrl) {
              try {
                ...
                console.warn("[NoGuidanceDemandGenService] Sitelink asset creation warning:", slErr?.response?.data || slErr.message);
              }
            }
          }
        }
          ...
          for (const co of callouts) {
            ...
            if (coText) {
              try {
                ...
                console.warn("[NoGuidanceDemandGenService] Callout asset warning:", e?.response?.data || e.message);
              }
            }
          }
        }
          ...
          for (const snip of structuredSnippets) {
            ...
            if (values.length > 0) {
              try {
                ...
                console.warn("[NoGuidanceDemandGenService] Structured snippet asset warning:", e?.response?.data || e.message);
              }
            }
          }
        }
          ...
          for (const promo of promotions) {
            ...
            if (promoTarget) {
              try {
                ...
                console.warn("[NoGuidanceDemandGenService] Promotion asset warning:", e?.response?.data || e.message);
              }
            }
          }
        }
        ...
        if (campaignAssetOperations.length > 0) {
          try {
            ...
            console.warn("[NoGuidanceDemandGenService] campaignAssets mutate warning:", caErr?.response?.data || caErr.message);
          }
        }
      }
      ...
      if (Array.isArray(conversionGoals) && conversionGoals.length > 0) {
        try {
          ...
          console.warn("[NoGuidanceDemandGenService] campaignConversionGoals mutate warning:", cgErr?.response?.data || cgErr.message);
        }
      }
      ...
      if (formatType === "VIDEO") {
        ...
        // If no explicit square marketing images were uploaded, require marketing image
        if (marketingImages.length === 0) {
          throw new Error("At least 1 marketing image is required for Demand Gen Single Image ads.");
        }

        // If square marketing images are empty, fallback to square marketing image or logo if valid square asset exists
    ...
    const localCampaign = await this.saveCampaignToDatabase({
      organizationId,
      customerId,
      googleAdsCampaignId: apiResult.campaignId || `noguidance-demandgen-${Date.now()}`,
      name: campaignName,
