...
export class LeadsDisplayService extends GoogleAdsBaseService {
  public static async createCampaign(organizationId: string, customerId: string, payload: any) {
    ...
    if (validHeadlines.length < 1) {
      validHeadlines.push("Quality Services & Fast Quotes");
    }

    const validDescriptions = descriptions.filter((d: any) => d && typeof d === "string" && d.trim().length > 0);
    if (validDescriptions.length < 1) {
      validDescriptions.push("Connect with top experts today to get high converting solutions.");
    }
    ...
    try {
      ...
      for (const img of rawImages) {
        ...
        if (fieldType === "MARKETING_IMAGE") {
          ...
          if (typeof raw === "string" && raw.includes("ik.imagekit.io")) {
            ...
            const ref = await this.uploadImageAsset(organizationId, customerId, `Leads_Disp_Sq_${Date.now()}`, raw);
      ...
      const safeHeadlines = (cleanedHeadlines.length > 0 ? cleanedHeadlines : ["Quality Services Near You"])
        .slice(0, 5)
      ...
      const cleanedLongHeadlines = validLongHeadlines
        ...
        .filter((text: string) => text.length > 0);
      const safeLongHeadline = cleanedLongHeadlines[0] || GoogleAdsBaseService.cleanAdText(validHeadlines[0] || "Connect with Experts and Receive Custom Quotes", 90);

      const cleanedDescriptions = validDescriptions
        .map((text: string) => GoogleAdsBaseService.cleanAdText(text, 90))
        .filter((text: string) => text.length > 0);
      const safeDescriptions = (cleanedDescriptions.length > 0 ? cleanedDescriptions : ["Get fast responses and verified solutions customized for your needs."])
        .slice(0, 5)
        .map((text: string) => ({ text }));
      ...
      console.error("[Google Ads API Error for Leads Display]:", formatted);
