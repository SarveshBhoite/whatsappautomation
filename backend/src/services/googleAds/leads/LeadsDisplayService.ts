...
export class LeadsDisplayService extends GoogleAdsBaseService {
  ...
  public static readonly CONTENT_LABEL_MAP: Record<string, string> = {
    "DL-G": "CONTENT_RATING_TIER_G",
    "DL-PG": "CONTENT_RATING_TIER_PG",
    "DL-T": "CONTENT_RATING_TIER_T",
    "DL-MA": "CONTENT_RATING_TIER_MA",
    "NOT_YET_LABELED": "CONTENT_RATING_TIER_NOT_YET_RATED",
    ...
    "Sensational and shocking": "SHOCKING",
    "Games": "GAMES",
    "Live streaming videos": "LIVE_STREAMING_VIDEO",
    "Embedded YouTube videos": "VIDEO_EMBEDDED",
    "Below-the-fold": "BELOW_THE_FOLD",
    "G-mob mobile app non interstitial": "GMOB_APP_NON_INTERSTITIAL",
  ...
  public static async resolveGeoTargetConstant(
    ...
    try {
      ...
      console.warn(`[LeadsDisplayService] geoTargetConstants:suggest failed for "${trimmed}":`, e?.message || e);
    }
    ...
    try {
      ...
      for (const source of allContentExclusionSources) {
        for (const [label, isExcluded] of Object.entries(source)) {
          if (isExcluded) {
            const mappedLabelEnum = LeadsDisplayService.CONTENT_LABEL_MAP[label];
            if (!mappedLabelEnum) {
              throw new Error(`Unsupported content label or exclusion: "${label}". Only official Google Ads ContentLabelTypeEnum values are allowed.`);
            }
      ...
      if (campaignCriterionOps.length > 0) {
        try {
          ...
          console.warn("[LeadsDisplayService] campaignCriteria mutate warning:", campCritErr?.response?.data || campCritErr.message);
      ...
      if (incomeSettings && typeof incomeSettings === "object") {
        for (const [tier, isIncluded] of Object.entries(incomeSettings)) {
          if (isIncluded === false) {
            const mappedIncome = LeadsDisplayService.INCOME_RANGE_MAP[tier];
            if (!mappedIncome) {
      ...
      if (adGroupCriterionOps.length > 0) {
        try {
          ...
          console.warn("[LeadsDisplayService] adGroupCriteria mutate warning:", critErr?.response?.data || critErr.message);
        ...
        for (const sl of sitelinks) {
          ...
          if (linkText && slUrl) {
            try {
              ...
              console.warn("[LeadsDisplayService] Sitelink asset creation warning:", slErr?.response?.data || slErr.message);
            }
          }
        }
      }
        ...
        for (const co of callouts) {
          ...
          if (coText) {
            try {
              ...
              console.warn("[LeadsDisplayService] Callout asset warning:", e?.response?.data || e.message);
            }
          }
        }
      }
        ...
        for (const snip of structuredSnippets) {
          ...
          if (values.length > 0) {
            try {
              ...
              console.warn("[LeadsDisplayService] Structured snippet asset warning:", e?.response?.data || e.message);
            }
          }
        }
      }
        ...
        for (const promo of promotions) {
          ...
          if (promoTarget) {
            try {
              ...
              console.warn("[LeadsDisplayService] Promotion asset warning:", e?.response?.data || e.message);
            }
          }
        }
      }
      ...
      if (callAsset && callAsset.phoneNumber) {
        try {
          ...
          console.warn("[LeadsDisplayService] Call asset warning:", e?.response?.data || e.message);
        }
      }
      ...
      if (leadFormAsset && leadFormAsset.businessName) {
        try {
          ...
          console.warn("[LeadsDisplayService] Lead form asset warning:", e?.response?.data || e.message);
        }
      }
      ...
      if (campaignAssetOperations.length > 0) {
        try {
          ...
          console.warn("[LeadsDisplayService] campaignAssets mutate warning:", caErr?.response?.data || caErr.message);
        }
      }
      ...
      if (Array.isArray(conversionGoals) && conversionGoals.length > 0) {
        try {
          ...
          console.warn("[LeadsDisplayService] campaignConversionGoals mutate warning:", cgErr?.response?.data || cgErr.message);
        }
      }
      ...
      if (callToAction && callToAction !== "Automated") {
        const ctaClean = String(callToAction).trim().toUpperCase().replace(/[\s-]+/g, "_");
        responsiveDisplayAd.callToActionText = ctaClean;
      }
