...
export class YoutubeDisplayLocalService extends GoogleAdsBaseService {
  public static async createCampaign(organizationId: string, customerId: string, payload: any) {
    ...
    try {
      ...
      const campaignRef = res.data?.results?.[0]?.resourceName || `customers/${cid}/campaigns/mock-yt-display-${Date.now()}`;
      ...
      try {
        ...
        apiResult.adGroupResourceName = adGroupRef;
         ...
         console.warn("[Google Ads API fallback for YouTube Display Ad Group]:", err.message);
      }
    } catch (apiErr: any) {
      console.warn("[Google Ads API fallback for YouTube Display]:", apiErr.message);
    }
