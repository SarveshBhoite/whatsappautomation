    ...
    try {
      ...
      try {
        ...
        if (err?.response?.data) {
          console.error("[Google Ads API Error for Website Traffic Search Ad Group]:", JSON.stringify(err.response.data, null, 2));
        } else {
          console.error("[Website Traffic Search Ad Group API Error]:", err.message);
        }
        throw err;
      }
    } catch (apiErr: any) {
