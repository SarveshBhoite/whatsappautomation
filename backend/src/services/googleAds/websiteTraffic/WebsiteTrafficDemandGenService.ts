...
export class WebsiteTrafficDemandGenService extends GoogleAdsBaseService {
  ...
  public static buildAudienceDimensions(audience: any, cid?: string): any[] {
    ...
    // Custom Segments
    if (Array.isArray(audience.customSegments)) {
      for (const cs of audience.customSegments) {
    ...
    if (Array.isArray(audience.yourData)) {
      for (const yd of audience.yourData) {
    ...
    // Lookalike Segments
    if (Array.isArray(audience.lookalikes)) {
      for (const lk of audience.lookalikes) {
  ...
  public static async resolveGeoTargetConstant(
    ...
    try {
      ...
      console.warn(`[WebsiteTrafficDemandGenService] geoTargetConstants:suggest failed for "${trimmed}":`, e?.message || e);
    }
  ...
  public static async createCampaign(organizationId: string, customerId: string, payload: any) {
    ...
    if (isAiGuided) {
      ...
      // Bidding validation
      const bStrat = (payload.biddingStrategy || payload.biddingFocus || "MAXIMIZE_CLICKS").trim().toUpperCase();
      if (bStrat === "YOUTUBE_ENGAGEMENTS" || bStrat === "YOUTUBE ENGAGEMENTS") {
        throw new Error("YouTube engagements bidding is not supported for Demand Gen Website Traffic campaigns in Google Ads API v24.");
      }
    ...
    const rawBStrat = (inputBiddingStrategy || (biddingFocus === "Target CPA" ? "TARGET_CPA" : biddingFocus === "Target ROAS" ? "TARGET_ROAS" : biddingFocus === "Conversions" ? "MAXIMIZE_CONVERSIONS" : "MAXIMIZE_CLICKS")).trim().toUpperCase();

    let biddingConfig: any = {};
    let finalBiddingStrategy = "MAXIMIZE_CLICKS";
    ...
    if (rawBStrat === "MAXIMIZE_CONVERSIONS" || rawBStrat === "CONVERSIONS") {
      ...
      // Default for Website Traffic Demand Gen is MAXIMIZE_CLICKS
    ...
    try {
      ...
      // ── 6. ATTACH CAMPAIGN-LEVEL CRITERIA (AD SCHEDULE & DEVICE TARGETING) ──
      const campaignCriterionOps: any[] = [];

      // A. Ad Schedule Criteria
      if (Array.isArray(adSchedule) && adSchedule.length > 0) {
        const scheduleCriteria = WebsiteTrafficDemandGenService.buildAdScheduleCriteria(adSchedule);
      ...
      if (campaignCriterionOps.length > 0) {
        try {
          ...
          console.warn("[WebsiteTrafficDemandGenService] campaignCriteria mutate warning:", campCritErr?.response?.data || campCritErr.message);
          if (isAiGuided) {
            throw campCritErr;
          }
        }
      ...
      // ── 7. CREATE AD GROUPS & CHANNEL CONTROLS ──
      ...
      if (audience && typeof audience === "object") {
        const dimensions = WebsiteTrafficDemandGenService.buildAudienceDimensions(audience, cid);
        if (dimensions.length > 0) {
          ...
          try {
              ...
              operations: [
                {
                  create: {
                    name: audienceName,
                    description: `Audience for Website Traffic Demand Gen Campaign ${effectiveCampaignName}`.slice(0, 255),
                    dimensions
                  }
                }
              ]
            ...
            console.warn("[WebsiteTrafficDemandGenService] Audience resource creation warning:", audErr?.response?.data || audErr.message);
            if (isAiGuided) {
              throw audErr;
            }
          }
      ...
      if (adGroupCriterionOps.length > 0) {
        try {
          ...
          console.warn("[WebsiteTrafficDemandGenService] adGroupCriteria mutate warning:", critErr?.response?.data || critErr.message);
          if (isAiGuided) {
            throw critErr;
          }
        }
      ...
      if (hasExtensions) {
        ...
        if (Array.isArray(sitelinks) && sitelinks.length > 0) {
          ...
          if (linkText && slUrl) {
            try {
              ...
              console.warn("[WebsiteTrafficDemandGenService] Sitelink asset creation warning:", slErr?.response?.data || slErr.message);
            }
          }
        }
      }
          ...
          for (const co of callouts) {
            ...
            if (coText) {
              try {
                ...
                console.warn("[WebsiteTrafficDemandGenService] Callout asset warning:", e?.response?.data || e.message);
              }
            }
          }
        }
          ...
          for (const snip of structuredSnippets) {
            ...
            if (values.length > 0) {
              try {
                ...
                console.warn("[WebsiteTrafficDemandGenService] Structured snippet asset warning:", e?.response?.data || e.message);
              }
            }
          }
        }
          ...
          for (const promo of promotions) {
            ...
            if (promoTarget) {
              try {
                ...
                console.warn("[WebsiteTrafficDemandGenService] Promotion asset warning:", e?.response?.data || e.message);
              }
            }
          }
        }
        ...
        if (campaignAssetOperations.length > 0) {
          try {
            ...
            console.warn("[WebsiteTrafficDemandGenService] campaignAssets mutate warning:", caErr?.response?.data || caErr.message);
          }
        }
      }
      ...
      if (Array.isArray(conversionGoals) && conversionGoals.length > 0) {
        try {
          ...
          console.warn("[WebsiteTrafficDemandGenService] campaignConversionGoals mutate warning:", cgErr?.response?.data || cgErr.message);
        }
      }
      ...
      const safeHeadlines = (validHeadlines.length > 0 ? validHeadlines : ["Explore Website Traffic", "Discover Top Products"])
        .slice(0, 5)
      ...
      console.error("[Google Ads API Error for Website Traffic Demand Gen]:", GoogleAdsBaseService.formatGoogleAdsError(apiErr));
