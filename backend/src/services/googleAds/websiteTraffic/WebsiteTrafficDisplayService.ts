...
export class WebsiteTrafficDisplayService extends GoogleAdsBaseService {
  public static async createCampaign(organizationId: string, customerId: string, payload: any) {
    ...
    if (validHeadlines.length < 1) {
      validHeadlines.push("Visit Our Official Website");
    }

    const validDescriptions = descriptions.filter((d: any) => d && typeof d === "string" && d.trim().length > 0);
    if (validDescriptions.length < 1) {
      validDescriptions.push("Explore all articles, product features, and helpful online guides.");
    }
    ...
    try {
      ...
      const euPoliticalValue = payload.euPolitical === "YES" ? "CONTAINS_EU_POLITICAL_ADVERTISING" : "DOES_NOT_CONTAIN_EU_POLITICAL_ADVERTISING";
      ...
      const rawLogos = (logos && logos.length > 0) ? logos : [
        "https://ik.imagekit.io/automationjds/tr:w-1200,h-1200,cm-pad_resize,bg-FFFFFF/gads_dg_logo_1787574973938_google_ads_logo_FJndWjppS.jpg"
      ];
      ...
      for (const img of rawImages) {
        ...
        if (fieldType === "MARKETING_IMAGE") {
          ...
          const landscapeRef = await this.uploadImageAsset(organizationId, customerId, `Web_Disp_Land_${Date.now()}`, landscapeUrl);
          ...
          const squareRef = await this.uploadImageAsset(organizationId, customerId, `Web_Disp_Sq_${Date.now()}`, squareUrl);
          ...
          const logoRef = await this.uploadImageAsset(organizationId, customerId, `Web_Disp_Logo_${Date.now()}`, logoUrl);
          ...
          if (typeof raw === "string" && raw.includes("ik.imagekit.io")) {
            ...
            const ref = await this.uploadImageAsset(organizationId, customerId, `Web_Disp_Sq_${Date.now()}`, raw);
      ...
      const safeHeadlines = (cleanedHeadlines.length > 0 ? cleanedHeadlines : ["Official Website Online"])
        .slice(0, 5)
      ...
      const cleanedLongHeadlines = validLongHeadlines
        ...
        .filter((text: string) => text.length > 0);
      const safeLongHeadline = cleanedLongHeadlines[0] || GoogleAdsBaseService.cleanAdText(validHeadlines[0] || "Visit Our Website and Explore Premium Solutions", 90);

      const cleanedDescriptions = validDescriptions
        .map((text: string) => GoogleAdsBaseService.cleanAdText(text, 90))
        .filter((text: string) => text.length > 0);
      const safeDescriptions = (cleanedDescriptions.length > 0 ? cleanedDescriptions : ["Read more and discover all services directly from our official website."])
        .slice(0, 5)
        .map((text: string) => ({ text }));
      ...
      console.error("[Google Ads API Error for Website Traffic Display]:", formatted);
