  ...
  public static async createCampaign(organizationId: string, customerId: string, payload: any) {
    ...
    try {
      ...
      for (const loc of locList) {
        ...
        const constantId = WebsiteTrafficPerformanceMaxService.GEO_TARGET_CONSTANT_MAP[normLoc] || (/^\d+$/.test(String(loc)) ? String(loc) : null);
        if (constantId) {
      ...
      for (const lang of langList) {
        ...
        const constantId = WebsiteTrafficPerformanceMaxService.LANGUAGE_CONSTANT_MAP[normLang] || (/^\d+$/.test(String(lang)) ? String(lang) : null);
        if (constantId) {
          criteriaOperations.push({
            create: {
              campaign: campaignRef,
      ...
      if (criteriaOperations.length > 0) {
        try {
          ...
          console.warn("[Website Traffic PMax] Warning: Failed to mutate campaign criteria:", critErr?.response?.data || critErr.message);
        }
      }
