...
async function runLeadsFullDiagnostic() {
  console.log("==================================================");
  console.log("🎯 COMPREHENSIVE LEADS CAMPAIGN DIAGNOSTIC");
  console.log("==================================================");

  const testResults = [];

  // STEP 1: Check Leads Campaign Schema Dropdown Options
  try {
    console.log("\n1️⃣ Testing GET /api/meta-ads/parameters (Dropdown Options Audit)...");
    ...
    console.log(`✅ Dropdown Audit: Lead Destinations (${destinationOpt.length}), Optimization Goals (${optimizationOpt.length}), Special Categories (${categoryOpt.length}), CTAs (${ctaOpt.length}).`);
  ...
  // STEP 2: Create Leads Campaign (Special Ad Category: EMPLOYMENT)
  let campaignId = null;
  let metaCampaignId = null;
  try {
    console.log("\n2️⃣ Testing POST /api/meta-ads/campaigns (Leads Objective + EMPLOYMENT Category)...");
    const createRes = await axios.post(`${BASE}/campaigns`, {
      organizationId: ORG_ID,
      name: `Full Diagnostic Leads Campaign - ${Date.now()}`,
      ...
      creativeHeadline: "Hiring Top Software Engineers & Automation Specialists",
      creativeBody: "Connect directly on WhatsApp with our HR recruiting team to apply.",
  ...
  try {
    ...
    if (found) {
      console.log(`✅ Passed: Created Leads campaign present in total ${list.length} listed campaigns.`);
  ...
  try {
    ...
    console.log(`✅ Passed: Campaign Name="${c.name}", Daily Budget=₹${c.dailyBudget}, Objective="${c.objective}", Category="${c.specialAdCategory}".`);
    testResults.push({ endpoint: "GET /campaigns/:id", status: "PASSED ✅", detail: `Budget: ₹${c.dailyBudget}, Category: ${c.specialAdCategory}` });
  } catch (err) {
  ...
  try {
    ...
    console.log(`✅ Passed: Selected Objective=${sv.objective}, Budget=₹${sv.dailyBudget}, Category=${sv.specialAdCategory}, Destination=${sv.destinationType}.`);
    testResults.push({ endpoint: "GET /campaigns/:id/parameters", status: "PASSED ✅", detail: `Verified Budget ₹${sv.dailyBudget}, Category ${sv.specialAdCategory}` });
  } catch (err) {
  ...
  console.log("\n==================================================");
  console.log("📊 LEADS CAMPAIGN ALL ENDPOINTS & DROPDOWNS RESULT:");
  console.log("==================================================");
  console.table(testResults);
}

runLeadsFullDiagnostic();
