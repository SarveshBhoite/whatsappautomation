...
describe("Meta Ads AI Assistant - Complete End-to-End User Flow Verification", () => {
  ...
  test("Simulates the exact user session from transcript end-to-end without bugs", async () => {
    ...
    state = await MetaAIConversationService.processMessage(orgId, state, "None", undefined, "NONE");
    aiMsg = state.conversation[state.conversation.length - 1];
    ...
    state = await MetaAIConversationService.processMessage(orgId, state, "WhatsApp Chat", undefined, "DESTINATION_WHATSAPP");
    aiMsg = state.conversation[state.conversation.length - 1];
    ...
    state = await MetaAIConversationService.processMessage(orgId, state, "+91 98765 43210", undefined, "USE_PHONE_919876543210");
    aiMsg = state.conversation[state.conversation.length - 1];
    ...
    state = await MetaAIConversationService.processMessage(orgId, state, "📍 All India (Poora Bharat)", undefined, "ALL_INDIA");
    aiMsg = state.conversation[state.conversation.length - 1];
    ...
    state = await MetaAIConversationService.processMessage(orgId, state, "👥 सभी लिंग (18 से 65 वर्ष)", undefined, "AGE_18_65_ALL");
    aiMsg = state.conversation[state.conversation.length - 1];
    ...
    state = await MetaAIConversationService.processMessage(orgId, state, "💼 Software, IT & Tech Business Owners", undefined, "INTEREST_TECH_BUSINESS");
    ...
    state = await MetaAIConversationService.processMessage(orgId, state, "₹500 / day (Advantage CBO)", undefined, "BUDGET_500");
